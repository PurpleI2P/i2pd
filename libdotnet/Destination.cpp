...
namespace client
{
		...
		m_LeaseSetType (DEFAULT_LEASESET_TYPE)
	{
		...
		std::shared_ptr<std::vector<dotnet::data::IdentHash> > explicitPeers;
		try
		{
			if (params)
			{
				auto it = params->find (DNCP_PARAM_INBOUND_TUNNEL_LENGTH);
				if (it != params->end ())
					inLen = std::stoi(it->second);
				it = params->find (DNCP_PARAM_OUTBOUND_TUNNEL_LENGTH);
				if (it != params->end ())
					outLen = std::stoi(it->second);
				it = params->find (DNCP_PARAM_INBOUND_TUNNELS_QUANTITY);
				if (it != params->end ())
					inQty = std::stoi(it->second);
				it = params->find (DNCP_PARAM_OUTBOUND_TUNNELS_QUANTITY);
				if (it != params->end ())
					outQty = std::stoi(it->second);
				it = params->find (DNCP_PARAM_TAGS_TO_SEND);
				if (it != params->end ())
				...
				it = params->find (DNCP_PARAM_EXPLICIT_PEERS);
				if (it != params->end ())
				{
					explicitPeers = std::make_shared<std::vector<dotnet::data::IdentHash> >();
				...
				it = params->find (DNCP_PARAM_INBOUND_NICKNAME);
				if (it != params->end ()) m_Nickname = it->second;
				else // try outbound
				{
					it = params->find (DNCP_PARAM_OUTBOUND_NICKNAME);
					if (it != params->end ()) m_Nickname = it->second;
					// otherwise we set default nickname in Start when we know local address
				}
				it = params->find (DNCP_PARAM_LEASESET_TYPE);
				if (it != params->end ())
		...
		SetNumTags (numTags);
		m_Pool = dotnet::tunnel::tunnels.CreateTunnelPool (inLen, outLen, inQty, outQty);
		if (explicitPeers)
			m_Pool->SetExplicitPeers (explicitPeers);
		if(params)
		{
			auto itr = params->find(DNCP_PARAM_MAX_TUNNEL_LATENCY);
			if (itr != params->end()) {
				auto maxlatency = std::stoi(itr->second);
				itr = params->find(DNCP_PARAM_MIN_TUNNEL_LATENCY);
				if (itr != params->end()) {
	...
	LeaseSetDestination::~LeaseSetDestination ()
	{
		if (m_IsRunning)
			Stop ();
		if (m_Pool)
			dotnet::tunnel::tunnels.DeleteTunnelPool (m_Pool);
	...
	bool LeaseSetDestination::Start ()
	{
		...
		{
			if (m_Nickname.empty ())
				m_Nickname = dotnet::data::GetIdentHashAbbreviation (GetIdentHash ()); // set default nickname
			LoadTags ();
			m_IsRunning = true;
	...
	bool LeaseSetDestination::Stop ()
	{
		if (m_IsRunning)
		{
			...
			if (m_Pool)
			{
				...
				dotnet::tunnel::tunnels.StopTunnelPool (m_Pool);
			}
			m_Service.stop ();
			if (m_Thread)
			{
	...
	bool LeaseSetDestination::Reconfigure(std::map<std::string, std::string> params)
	{
		...
		auto itr = params.find("dncp.dontPublishLeaseSet");
		if (itr != params.end())
		{
		...
		std::map<std::string, int&> intOpts = {
			{DNCP_PARAM_INBOUND_TUNNEL_LENGTH, inLen},
			{DNCP_PARAM_OUTBOUND_TUNNEL_LENGTH, outLen},
			{DNCP_PARAM_INBOUND_TUNNELS_QUANTITY, inQuant},
			{DNCP_PARAM_OUTBOUND_TUNNELS_QUANTITY, outQuant},
			{DNCP_PARAM_TAGS_TO_SEND, numTags},
			{DNCP_PARAM_MIN_TUNNEL_LATENCY, minLatency},
			{DNCP_PARAM_MAX_TUNNEL_LATENCY, maxLatency}
		};
	...
	std::shared_ptr<dotnet::data::LeaseSet> LeaseSetDestination::FindLeaseSet (const dotnet::data::IdentHash& ident)
	{
		std::shared_ptr<dotnet::data::LeaseSet> remoteLS;
		{
		...
		if (remoteLS)
		{
			if (!remoteLS->IsExpired ())
			{
				if (remoteLS->ExpiresSoon())
				{
					...
					RequestDestination(ident, [s, ident] (std::shared_ptr<dotnet::data::LeaseSet> ls) {
						if(ls && !ls->IsExpired())
						{
							ls->PopulateLeases();
							{
		...
		{
			auto ls = dotnet::data::netdb.FindLeaseSet (ident);
			if (ls && !ls->IsExpired ())
			{
	...
	std::shared_ptr<const dotnet::data::LocalLeaseSet> LeaseSetDestination::GetLeaseSet ()
	{
	...
	std::shared_ptr<const dotnet::data::LocalLeaseSet> LeaseSetDestination::GetLeaseSetMt ()
	{
		std::lock_guard<std::mutex> l(m_LeaseSetMutex);
		return m_LeaseSet;
	}
		
	void LeaseSetDestination::SetLeaseSet (std::shared_ptr<const dotnet::data::LocalLeaseSet> newLeaseSet)
	{
		{
			std::lock_guard<std::mutex> l(m_LeaseSetMutex);
			m_LeaseSet = newLeaseSet;
		}
		dotnet::garlic::GarlicDestination::SetLeaseSetUpdated ();
		if (m_IsPublic)
		{
			auto s = shared_from_this ();
	...
	void LeaseSetDestination::UpdateLeaseSet ()
	{
		...
		if (numTunnels > dotnet::data::MAX_NUM_LEASES) numTunnels = dotnet::data::MAX_NUM_LEASES; // 16 tunnels maximum
	...
	void LeaseSetDestination::ProcessGarlicMessage (std::shared_ptr<DNNPMessage> msg)
	{
		m_Service.post (std::bind (&LeaseSetDestination::HandleGarlicMessage, shared_from_this (), msg));
	}

	void LeaseSetDestination::ProcessDeliveryStatusMessage (std::shared_ptr<DNNPMessage> msg)
	{
		m_Service.post (std::bind (&LeaseSetDestination::HandleDeliveryStatusMessage, shared_from_this (), msg));
	}

	void LeaseSetDestination::HandleDNNPMessage (const uint8_t * buf, size_t len, std::shared_ptr<dotnet::tunnel::InboundTunnel> from)
	{
		uint8_t typeID = buf[DNNP_HEADER_TYPEID_OFFSET];
		switch (typeID)
		{
			case eDNNPData:
				HandleDataMessage (buf + DNNP_HEADER_SIZE, GetDNNPMessageLength(buf, len) - DNNP_HEADER_SIZE);
			break;
			case eDNNPDeliveryStatus:
				// we assume tunnel tests non-encrypted
				HandleDeliveryStatusMessage (CreateDNNPMessage (buf, GetDNNPMessageLength (buf, len), from));
			break;
			case eDNNPDatabaseStore:
				HandleDatabaseStoreMessage (buf + DNNP_HEADER_SIZE, GetDNNPMessageLength(buf, len) - DNNP_HEADER_SIZE);
			break;
			case eDNNPDatabaseSearchReply:
				HandleDatabaseSearchReplyMessage (buf + DNNP_HEADER_SIZE, GetDNNPMessageLength(buf, len) - DNNP_HEADER_SIZE);
			break;
			default:
	...
	void LeaseSetDestination::HandleDatabaseStoreMessage (const uint8_t * buf, size_t len)
	{
		...
		dotnet::data::IdentHash key (buf + DATABASE_STORE_KEY_OFFSET);
		std::shared_ptr<dotnet::data::LeaseSet> leaseSet;
		switch (buf[DATABASE_STORE_TYPE_OFFSET])
		{
			case dotnet::data::NETDB_STORE_TYPE_LEASESET: // 1
			case dotnet::data::NETDB_STORE_TYPE_STANDARD_LEASESET2: // 3
			{
				...
				{
					if (buf[DATABASE_STORE_TYPE_OFFSET] == dotnet::data::NETDB_STORE_TYPE_LEASESET)
						leaseSet = std::make_shared<dotnet::data::LeaseSet> (buf + offset, len - offset); // LeaseSet
					else
						leaseSet = std::make_shared<dotnet::data::LeaseSet2> (buf[DATABASE_STORE_TYPE_OFFSET], buf + offset, len - offset); // LeaseSet2
					if (leaseSet->IsValid () && leaseSet->GetIdentHash () == key)
					{
			...
			case dotnet::data::NETDB_STORE_TYPE_ENCRYPTED_LEASESET2: // 5
			{
				auto it2 = m_LeaseSetRequests.find (key);
				if (it2 != m_LeaseSetRequests.end () && it2->second->requestedBlindedKey)
				{
					auto ls2 = std::make_shared<dotnet::data::LeaseSet2> (buf + offset, len - offset, it2->second->requestedBlindedKey);
					if (ls2->IsValid ())
					{
	...
	void LeaseSetDestination::HandleDatabaseSearchReplyMessage (const uint8_t * buf, size_t len)
	{
		...
		if (it != m_LeaseSetRequests.end ())
		{
			...
			if (request->excluded.size () < MAX_NUM_FLOODFILLS_PER_REQUEST)
			{
				for (int i = 0; i < num; i++)
			{
				dotnet::data::IdentHash peerHash (buf + 33 + i*32);
				if (!request->excluded.count (peerHash) && !dotnet::data::netdb.FindRouter (peerHash))
				{
					LogPrint (eLogInfo, "Destination: Found new floodfill, request it"); // TODO: recheck this message
					dotnet::data::netdb.RequestDestination (peerHash);
				}
			}

				auto floodfill = dotnet::data::netdb.GetClosestFloodfill (key, request->excluded);
				if (floodfill)
				{
	...
	void LeaseSetDestination::HandleDeliveryStatusMessage (std::shared_ptr<DNNPMessage> msg)
	{
		...
		if (msgID == m_PublishReplyToken)
		{
			...
			dotnet::garlic::GarlicDestination::HandleDeliveryStatusMessage (msg);
	}
	...
	void LeaseSetDestination::Publish ()
	{
		...
		auto floodfill = dotnet::data::netdb.GetClosestFloodfill (leaseSet->GetIdentHash (), m_ExcludedFloodfills);
		if (!floodfill)
		{
		...
		auto msg = WrapMessage (floodfill, dotnet::CreateDatabaseStoreMsg (leaseSet, m_PublishReplyToken, inbound));
	...
	void LeaseSetDestination::HandlePublishConfirmationTimer (const boost::system::error_code& ecode)
	{
		if (ecode != boost::asio::error::operation_aborted)
		{
			...
			{
				m_PublishReplyToken = 0;
				if (GetIdentity ()->GetCryptoKeyType () == dotnet::data::CRYPTO_KEY_TYPE_ELGAMAL)
				{
	...
	void LeaseSetDestination::HandlePublishVerificationTimer (const boost::system::error_code& ecode)
	{
		if (ecode != boost::asio::error::operation_aborted)
		{
			...
			RequestLeaseSet (ls->GetStoreHash (),
				[s, ls, this](std::shared_ptr<const dotnet::data::LeaseSet> leaseSet)
				{
					if (leaseSet)
					{
						if (*ls == *leaseSet)
	...
	bool LeaseSetDestination::RequestDestination (const dotnet::data::IdentHash& dest, RequestComplete requestComplete)
	{
		if (!m_Pool || !IsReady ())
		{
			if (requestComplete)
	...
	bool LeaseSetDestination::RequestDestinationWithEncryptedLeaseSet (std::shared_ptr<const dotnet::data::BlindedPublicKey> dest, RequestComplete requestComplete)
	{
	...
	void LeaseSetDestination::CancelDestinationRequest (const dotnet::data::IdentHash& dest, bool notify)
	{
		auto s = shared_from_this ();
	...
	void LeaseSetDestination::CancelDestinationRequestWithEncryptedLeaseSet (std::shared_ptr<const dotnet::data::BlindedPublicKey> dest, bool notify)
	{
		if (dest)
			CancelDestinationRequest (dest->GetStoreHash (), notify);
	}

	void LeaseSetDestination::RequestLeaseSet (const dotnet::data::IdentHash& dest, RequestComplete requestComplete, std::shared_ptr<const dotnet::data::BlindedPublicKey> requestedBlindedKey)
	{
		std::set<dotnet::data::IdentHash> excluded;
		auto floodfill = dotnet::data::netdb.GetClosestFloodfill (dest, excluded);
		if (floodfill)
		{
			...
			auto ts = dotnet::util::GetSecondsSinceEpoch ();
			auto ret = m_LeaseSetRequests.insert (std::pair<dotnet::data::IdentHash, std::shared_ptr<LeaseSetRequest> >(dest,request));
	...
	bool LeaseSetDestination::SendLeaseSetRequest (const dotnet::data::IdentHash& dest,
		std::shared_ptr<const dotnet::data::RouterInfo>  nextFloodfill, std::shared_ptr<LeaseSetRequest> request)
	{
		...
		if (request->replyTunnel && request->outboundTunnel)
		{
			...
			request->outboundTunnel->SendTunnelDataMsg (
				{
					dotnet::tunnel::TunnelMessageBlock
					{
						dotnet::tunnel::eDeliveryTypeRouter,
	...
	void LeaseSetDestination::HandleRequestTimoutTimer (const boost::system::error_code& ecode, const dotnet::data::IdentHash& dest)
	{
		if (ecode != boost::asio::error::operation_aborted)
		{
			...
			if (it != m_LeaseSetRequests.end ())
			{
				...
				{
					auto floodfill = dotnet::data::netdb.GetClosestFloodfill (dest, it->second->excluded);
					if (floodfill)
					{
	...
	ClientDestination::ClientDestination (const dotnet::data::PrivateKeys& keys, bool isPublic, const std::map<std::string, std::string> * params):
		LeaseSetDestination (isPublic, params), m_Keys (keys), m_StreamingAckDelay (DEFAULT_INITIAL_ACK_DELAY),
		m_DatagramDestination (nullptr), m_RefCounter (0),
		m_ReadyChecker(GetService())
	{
		if (keys.IsOfflineSignature () && GetLeaseSetType () == dotnet::data::NETDB_STORE_TYPE_LEASESET)
			SetLeaseSetType (dotnet::data::NETDB_STORE_TYPE_STANDARD_LEASESET2); // offline keys can be published with LS2 only

		m_EncryptionKeyType = GetIdentity ()->GetCryptoKeyType ();
		// extract encryption type params for LS2
		if (GetLeaseSetType () == dotnet::data::NETDB_STORE_TYPE_STANDARD_LEASESET2 && params)
		{
			auto it = params->find (DNCP_PARAM_LEASESET_ENCRYPTION_TYPE);
			if (it != params->end ())
		...
		if (isPublic && m_EncryptionKeyType == GetIdentity ()->GetCryptoKeyType ()) // TODO: presist key type
			...
			dotnet::data::PrivateKeys::GenerateCryptoKeyPair (m_EncryptionKeyType, m_EncryptionPrivateKey, m_EncryptionPublicKey);
		m_Decryptor = dotnet::data::PrivateKeys::CreateDecryptor (m_EncryptionKeyType, m_EncryptionPrivateKey);
		if (isPublic)
		...
		if (params)
		{
			auto it = params->find (DNCP_PARAM_STREAMING_INITIAL_ACK_DELAY);
			if (it != params->end ())
	...
	bool ClientDestination::Start ()
	{
		...
		{
			m_StreamingDestination = std::make_shared<dotnet::stream::StreamingDestination> (GetSharedFromThis ()); // TODO:
	...
	void ClientDestination::CreateStream (StreamRequestComplete streamRequestComplete, const dotnet::data::IdentHash& dest, int port)
	{
		...
		else
		{
			...
			RequestDestination (dest,
				[s, streamRequestComplete, port](std::shared_ptr<const dotnet::data::LeaseSet> ls)
				{
					if (ls)
	...
	void ClientDestination::CreateStream (StreamRequestComplete streamRequestComplete, std::shared_ptr<const dotnet::data::BlindedPublicKey> dest, int port)
	{
		if (!streamRequestComplete)
		{
			LogPrint (eLogError, "Destination: request callback is not specified in CreateStream");
		...
		RequestDestinationWithEncryptedLeaseSet (dest,
			[s, streamRequestComplete, port](std::shared_ptr<dotnet::data::LeaseSet> ls)
			{
				if (ls)
					streamRequestComplete(s->CreateStream (ls, port));
				else
	...
	std::shared_ptr<dotnet::stream::Stream> ClientDestination::CreateStream (std::shared_ptr<const dotnet::data::LeaseSet> remote, int port)
	{
	...
	std::shared_ptr<dotnet::stream::StreamingDestination> ClientDestination::GetStreamingDestination (int port) const
	{
		if (port)
		{
	...
	void ClientDestination::AcceptStreams (const dotnet::stream::StreamingDestination::Acceptor& acceptor)
	{
		if (m_StreamingDestination)
	...
	void ClientDestination::AcceptOnce (const dotnet::stream::StreamingDestination::Acceptor& acceptor)
	{
		if (m_StreamingDestination)
			m_StreamingDestination->AcceptOnce (acceptor);
	}

	std::shared_ptr<dotnet::stream::StreamingDestination> ClientDestination::CreateStreamingDestination (int port, bool gzip)
	{
		auto dest = std::make_shared<dotnet::stream::StreamingDestination> (GetSharedFromThis (), port, gzip);
		if (port)
  ...
  dotnet::datagram::DatagramDestination * ClientDestination::CreateDatagramDestination ()
	{
		if (m_DatagramDestination == nullptr)
			m_DatagramDestination = new dotnet::datagram::DatagramDestination (GetSharedFromThis ());
		return m_DatagramDestination;
	}

	std::vector<std::shared_ptr<const dotnet::stream::Stream> > ClientDestination::GetAllStreams () const
	{
		std::vector<std::shared_ptr<const dotnet::stream::Stream> > ret;
		if (m_StreamingDestination)
		{
	...
	void ClientDestination::PersistTemporaryKeys ()
	{
		...
		std::string path  = dotnet::fs::DataDirPath("destinations", (ident + ".dat"));
	...
	void ClientDestination::CreateNewLeaseSet (std::vector<std::shared_ptr<dotnet::tunnel::InboundTunnel> > tunnels)
	{
		std::shared_ptr<dotnet::data::LocalLeaseSet> leaseSet;
		if (GetLeaseSetType () == dotnet::data::NETDB_STORE_TYPE_LEASESET)
		{
			leaseSet = std::make_shared<dotnet::data::LocalLeaseSet> (GetIdentity (), m_EncryptionPublicKey, tunnels);
			// sign
		...
		else
		{
			...
			auto ls2 = std::make_shared<dotnet::data::LocalLeaseSet2> (dotnet::data::NETDB_STORE_TYPE_STANDARD_LEASESET2,
				m_Keys, m_EncryptionKeyType, keyLen, m_EncryptionPublicKey, tunnels);
			if (GetLeaseSetType () == dotnet::data::NETDB_STORE_TYPE_ENCRYPTED_LEASESET2) // encrypt if type 5
				ls2 = std::make_shared<dotnet::data::LocalEncryptedLeaseSet2> (ls2, m_Keys);
			leaseSet = ls2;
		}
