...
namespace stream
{
	...
	Stream::Stream (boost::asio::io_service& service, StreamingDestination& local,
		std::shared_ptr<const dotnet::data::LeaseSet> remote, int port): m_Service (service),
	...
	bool Stream::ProcessOptions (uint16_t flags, Packet * packet)
	{
		...
		if (flags & PACKET_FLAG_FROM_INCLUDED)
		{
			...
			if (!m_RemoteIdentity)
				m_RemoteIdentity = std::make_shared<dotnet::data::IdentityEx>(optionData, optionSize);
		...
		if (flags & PACKET_FLAG_OFFLINE_SIGNATURE)
		{
			...
			else
			{
				// transient key
				size_t offset = 0;
				m_TransientVerifier = dotnet::data::ProcessOfflineSignature (m_RemoteIdentity, optionData, optionSize - (optionData - packet->GetOptionData ()), offset);
				optionData += offset;
				if (!m_TransientVerifier) 
				{
			...
			if (seqn <= ackThrough)
			{
				...
				if (!seqn && m_RoutingSession) // first message confirmed
					m_RoutingSession->SetSharedRoutingPath (
						std::make_shared<dotnet::garlic::GarlicRoutingPath> (
							dotnet::garlic::GarlicRoutingPath{m_CurrentOutboundTunnel, m_CurrentRemoteLease, m_RTT, 0, 0}));
			}
			else
				break;
		}
	...
	void Stream::SendPackets (const std::vector<Packet *>& packets)
	{
		...
		if (!m_CurrentRemoteLease || !m_CurrentRemoteLease->endDate || // excluded from LeaseSet
		    ts >= m_CurrentRemoteLease->endDate - dotnet::data::LEASE_ENDDATE_THRESHOLD)
			UpdateCurrentRemoteLease (true);
		if (m_CurrentRemoteLease && ts < m_CurrentRemoteLease->endDate + dotnet::data::LEASE_ENDDATE_THRESHOLD)
		{
	...
	void Stream::SendUpdatedLeaseSet ()
	{
		if (m_RoutingSession)
		{
			...
			{
				auto ts = dotnet::util::GetMillisecondsSinceEpoch ();
				if (ts > m_RoutingSession->GetLeaseSetSubmissionTime () + dotnet::garlic::LEASET_CONFIRMATION_TIMEOUT)
				{
					// LeaseSet was not confirmed, should try other tunnels
					LogPrint (eLogWarning, "Streaming: LeaseSet was not confirmed in ", dotnet::garlic::LEASET_CONFIRMATION_TIMEOUT,  " milliseconds. Trying to resubmit");
	...
	void Stream::UpdateCurrentRemoteLease (bool expired)
	{
		...
		if (m_RemoteLeaseSet)
		{
			...
			if (leases.empty ())
			{
				expired = false;
				// time to request
				if (m_RemoteLeaseSet->GetOrigStoreType () == dotnet::data::NETDB_STORE_TYPE_ENCRYPTED_LEASESET2)
					m_LocalDestination.GetOwner ()->RequestDestinationWithEncryptedLeaseSet (
						std::make_shared<dotnet::data::BlindedPublicKey>(m_RemoteIdentity));
				else
	...
	StreamingDestination::StreamingDestination (std::shared_ptr<dotnet::client::ClientDestination> owner, uint16_t localPort, bool gzip):
	...
	std::shared_ptr<Stream> StreamingDestination::CreateNewOutgoingStream (std::shared_ptr<const dotnet::data::LeaseSet> remote, int port)
	{
	...
	std::shared_ptr<DNNPMessage> StreamingDestination::CreateDataMessage (const uint8_t * payload, size_t len, uint16_t toPort)
	{
		auto msg = NewDNNPShortMessage ();
		if (!m_Gzip || len <= dotnet::stream::COMPRESSION_THRESHOLD_SIZE)
		...
		if (size)
		{
			...
			buf[9] = dotnet::client::PROTOCOL_TYPE_STREAMING; // streaming protocol
			msg->len += size;
