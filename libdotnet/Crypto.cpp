...
// HMAC
	...
	// block size is 64 bytes
	{
...
#endif
		{
			...
			hash[6] = OPAD;
			hash[7] = OPAD;
			// fill last 16 bytes with zeros (first hash size assumed 32 bytes in DOTNET)
			memset (hash + 10, 0, 16);
		}
...
// AES
	...
	void TunnelEncryption::Encrypt (const uint8_t * in, uint8_t * out)
	{
...
#endif
		{
			...
			m_LayerEncryption.Encrypt (in + 16, dotnet::tunnel::TUNNEL_DATA_ENCRYPTED_SIZE, out + 16); // data
	...
	void TunnelDecryption::Decrypt (const uint8_t * in, uint8_t * out)
	{
...
#endif
		{
			...
			m_LayerDecryption.Decrypt (in + 16, dotnet::tunnel::TUNNEL_DATA_ENCRYPTED_SIZE, out + 16); // data
