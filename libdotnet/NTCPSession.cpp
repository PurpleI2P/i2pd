...
namespace transport
{
	...
	NTCPSession::NTCPSession (NTCPServer& server, std::shared_ptr<const dotnet::data::RouterInfo> in_RemoteRouter):
	...
	void NTCPSession::HandlePhase1Received (const boost::system::error_code& ecode, std::size_t bytes_transferred)
	{
		...
		else
		{
			...
			const uint8_t * ident = dotnet::context.GetIdentHash ();
			for (int i = 0; i < 32; i++)
			{
	...
	void NTCPSession::SendPhase2 (NTCPWork * work)
	{
		...
		uint32_t tsB = htobe32 (dotnet::util::GetSecondsSinceEpoch ());
	...
	void NTCPSession::HandlePhase2Received (const boost::system::error_code& ecode, std::size_t bytes_transferred)
	{
		...
		if (ecode)
		{
			...
			if (ecode != boost::asio::error::operation_aborted)
			{
				...
				dotnet::data::netdb.SetUnreachable (GetRemoteIdentity ()->GetIdentHash (), true);
	...
	{
		auto& keys = dotnet::context.GetPrivateKeys ();
		uint8_t * buf = m_ReceiveBuffer;
		htobe16buf (buf, keys.GetPublic ()->GetFullLen ());
		buf += 2;
		buf += dotnet::context.GetIdentity ()->ToBuffer (buf, NTCP_BUFFER_SIZE);
		uint32_t tsA = htobe32 (dotnet::util::GetSecondsSinceEpoch ());
		htobuf32(buf,tsA);
		buf += 4;
	...
	void NTCPSession::HandlePhase3Received (const boost::system::error_code& ecode, std::size_t bytes_transferred, uint32_t tsB)
	{
		...
		else
		{
			...
			auto identity = std::make_shared<dotnet::data::IdentityEx> (buf + 2, size);
	...
	void NTCPSession::HandlePhase3 (uint32_t tsB, size_t paddingLen)
	{
		...
		s.Insert (m_Establisher->phase1.pubKey, 256); // x
		s.Insert (m_Establisher->phase2.pubKey, 256); // y
		s.Insert (dotnet::context.GetRouterInfo ().GetIdentHash (), 32); // ident
		s.Insert (tsA); // tsA
		s.Insert (tsB); // tsB
	...
	void NTCPSession::HandlePhase4Received (const boost::system::error_code& ecode, std::size_t bytes_transferred, uint32_t tsA)
	{
		...
		else
		{
			...
			s.Insert (m_Establisher->phase1.pubKey, 256); // x
			s.Insert (m_Establisher->phase2.pubKey, 256); // y
			s.Insert (dotnet::context.GetIdentHash (), 32); // ident
			s.Insert (tsA); // tsA
			s.Insert (m_Establisher->phase2.encrypted.timestamp, 4); // tsB
	...
	void NTCPSession::HandleReceived (const boost::system::error_code& ecode, std::size_t bytes_transferred)
	{
		...
		else
		{
			...
			if (moreBytes && !ec)
			{
				...
				dotnet::transport::transports.UpdateReceivedBytes (moreBytes);
				// process more data
	...
	bool NTCPSession::DecryptNextBlock (const uint8_t * encrypted) // 16 bytes
	{
		if (!m_NextMessage) // new message, header expected
		{
			...
			if (dataSize)
			{
				...
				m_NextMessage = (dataSize + 16U + 15U) <= DNNP_MAX_SHORT_MESSAGE_SIZE - 2 ? NewDNNPShortMessage () : NewDNNPMessage ();
			...
			{
				// timestamp
				int diff = (int)bufbe32toh (buf + 2) - (int)dotnet::util::GetSecondsSinceEpoch ();
		...
		{
			// we have a complete DNNP message
			uint8_t checksum[4];
	...
	void NTCPSession::Send (std::shared_ptr<dotnet::DNNPMessage> msg)
	{
		m_IsSending = true;
		boost::asio::async_write (m_Socket, CreateMsgBuffer (msg), boost::asio::transfer_all (),
			std::bind(&NTCPSession::HandleSent, shared_from_this (), std::placeholders::_1, std::placeholders::_2, std::vector<std::shared_ptr<DNNPMessage> >{ msg }));
	}

	boost::asio::const_buffers_1 NTCPSession::CreateMsgBuffer (std::shared_ptr<DNNPMessage> msg)
	{
		uint8_t * sendBuffer;
		int len;
		...
		if (msg)
		{
			// regular DNNP
			if (msg->offset < 2)
				LogPrint (eLogError, "NTCP: Malformed DNNP message"); // TODO:
		...
		else
		{
			...
			htobe32buf (sendBuffer + 2, dotnet::util::GetSecondsSinceEpoch ());
		}
	...
	void NTCPSession::Send (const std::vector<std::shared_ptr<DNNPMessage> >& msgs)
	{
		m_IsSending = true;
	...
	void NTCPSession::HandleSent (const boost::system::error_code& ecode, std::size_t bytes_transferred, std::vector<std::shared_ptr<DNNPMessage> > msgs)
	{
		(void) msgs;
		m_IsSending = false;
		if (ecode)
	...
	void NTCPSession::SendDNNPMessages (const std::vector<std::shared_ptr<DNNPMessage> >& msgs)
	{
		m_Server.GetService ().post (std::bind (&NTCPSession::PostDNNPMessages, shared_from_this (), msgs));
	}

	void NTCPSession::PostDNNPMessages (std::vector<std::shared_ptr<DNNPMessage> > msgs)
	{
...
//-----------------------------------------
	...
	void NTCPServer::Start ()
	{
		if (!m_IsRunning)
		{
			...
			else
			{
				...
				{
					if (!address) continue;
					if (address->transportStyle == dotnet::data::RouterInfo::eTransportNTCP && !address->IsNTCP2 ())
					{
	...
	bool NTCPServer::AddNTCPSession (std::shared_ptr<NTCPSession> session)
	{
		...
		m_NTCPSessions.insert (std::pair<dotnet::data::IdentHash, std::shared_ptr<NTCPSession> >(ident, session));
		return true;
	}
	...
	std::shared_ptr<NTCPSession> NTCPServer::FindNTCPSession (const dotnet::data::IdentHash& ident)
	{
		auto it = m_NTCPSessions.find (ident);
		if (it != m_NTCPSessions.end ())
			return it->second;
