...
namespace transport
{
	SSUSession::SSUSession (SSUServer& server, boost::asio::ip::udp::endpoint& remoteEndpoint,
		std::shared_ptr<const dotnet::data::RouterInfo> router, bool peerTest ):
		...
		m_RelayTag (0), m_SentRelayTag (0), m_Data (*this), m_IsDataReceived (false)
	{
		...
		{
			// we are server
			auto address = dotnet::context.GetRouterInfo ().GetSSUAddress (false);
			if (address) m_IntroKey = address->ssu->key;
		}
	...
	void SSUSession::ProcessNextMessage (uint8_t * buf, size_t len, const boost::asio::ip::udp::endpoint& senderEndpoint)
	{
		...
		dotnet::transport::transports.UpdateReceivedBytes (len);
	...
	void SSUSession::ProcessSessionCreated (uint8_t * buf, size_t len)
	{
		...
		if (dotnet::context.GetStatus () == eRouterStatusTesting)
		{
			...
			if (signedOnTime < ts - SSU_CLOCK_SKEW || signedOnTime > ts + SSU_CLOCK_SKEW)
			{
				...
				dotnet::context.SetError (eRouterErrorClockSkew);
			}
		}
		...
		if (s.Verify (m_RemoteIdentity, payload))
		{
			...
			dotnet::context.UpdateAddress (ourIP);
	...
	void SSUSession::ProcessSessionConfirmed (const uint8_t * buf, size_t len)
	{
		...
		auto identity = std::make_shared<dotnet::data::IdentityEx> (payload, identitySize);
		auto existing = dotnet::data::netdb.FindRouter (identity->GetIdentHash ()); // check if exists already
	...
	void SSUSession::SendSessionRequest ()
	{
		...
		if (dotnet::context.GetStatus () == eRouterStatusOK) // we don't need relays
		{
	...
	void SSUSession::SendRelayRequest (const dotnet::data::RouterInfo::Introducer& introducer, uint32_t nonce)
	{
		auto address = dotnet::context.GetRouterInfo ().GetSSUAddress (false);
		if (!address)
		{
	...
	{
		auto address = IsV6 () ? dotnet::context.GetRouterInfo ().GetSSUV6Address () :
			dotnet::context.GetRouterInfo ().GetSSUAddress (true); //v4 only
		if (!address)
		{
			LogPrint (eLogInfo, "SSU is not supported");
			return;
		...
		if (sendRelayTag && dotnet::context.GetRouterInfo ().IsIntroducer () && !IsV6 ())
		{
		...
		payload += 4; // relay tag
		htobe32buf (payload, dotnet::util::GetSecondsSinceEpoch ()); // signed on time
		payload += 4;
		...
		s.Sign (dotnet::context.GetPrivateKeys (), payload); // DSA signature

		uint8_t iv[16];
		RAND_bytes (iv, 16); // random iv
		// encrypt signature and padding with newly created session key
		size_t signatureLen = dotnet::context.GetIdentity ()->GetSignatureLen ();
		size_t paddingSize = signatureLen & 0x0F; // %16
		if (paddingSize > 0)
		{
			// fill random padding
	...
	void SSUSession::SendSessionConfirmed (const uint8_t * y, const uint8_t * ourAddress, size_t ourAddressLen)
	{
		...
		*payload = 1; // 1 fragment
		payload++; // info
		size_t identLen = dotnet::context.GetIdentity ()->GetFullLen (); // 387+ bytes
		htobe16buf (payload, identLen);
		payload += 2; // cursize
		dotnet::context.GetIdentity ()->ToBuffer (payload, identLen);
		payload += identLen;
		uint32_t signedOnTime = dotnet::util::GetSecondsSinceEpoch ();
		htobe32buf (payload, signedOnTime); // signed on time
		payload += 4;
		auto signatureLen = dotnet::context.GetIdentity ()->GetSignatureLen ();
	...
	void SSUSession::FillHeaderAndEncrypt (uint8_t payloadType, uint8_t * buf, size_t len,
		const dotnet::crypto::AESKey& aesKey, const uint8_t * iv, const dotnet::crypto::MACKey& macKey, uint8_t flag)
	{
		...
		htobe32buf (header->time, dotnet::util::GetSecondsSinceEpoch ());
		...
		dotnet::crypto::HMACMD5Digest (encrypted, encryptedLen + 18, macKey, header->mac);
	}
	...
	void SSUSession::FillHeaderAndEncrypt (uint8_t payloadType, uint8_t * buf, size_t len)
	{
		...
		htobe16buf (buf + len + 16, encryptedLen);
		dotnet::crypto::HMACMD5Digest (encrypted, encryptedLen + 18, m_MacKey, header->mac);
	}

	void SSUSession::Decrypt (uint8_t * buf, size_t len, const dotnet::crypto::AESKey& aesKey)
	{
		if (len < sizeof (SSUHeader))
		{
			LogPrint (eLogError, "SSU: Unexpected packet length ", len);
	...
	bool SSUSession::Validate (uint8_t * buf, size_t len, const dotnet::crypto::MACKey& macKey)
	{
		if (len < sizeof (SSUHeader))
		{
			LogPrint (eLogError, "SSU: Unexpected packet length ", len);
		...
		htobe16buf (buf + len + 16, encryptedLen);
		uint8_t digest[16];
		dotnet::crypto::HMACMD5Digest (encrypted, encryptedLen + 18, macKey, digest);
	...
	void SSUSession::Introduce (const dotnet::data::RouterInfo::Introducer& introducer,
		std::shared_ptr<const dotnet::data::RouterInfo> to)
	{
		if (m_State == eSessionStateUnknown)
		{
			// set connect timer
	...
	void SSUSession::SendDNNPMessages (const std::vector<std::shared_ptr<DNNPMessage> >& msgs)
	{
		GetService ().post (std::bind (&SSUSession::PostDNNPMessages, shared_from_this (), msgs));
	}

	void SSUSession::PostDNNPMessages (std::vector<std::shared_ptr<DNNPMessage> > msgs)
	{
		if (m_State == eSessionStateEstablished)
		{
			for (const auto& it: msgs)
				if (it)
				{
					if (it->GetLength () <= SSU_MAX_DNNP_MESSAGE_SIZE) 
						m_Data.Send (it);
					else
						LogPrint (eLogError, "SSU: DNNP message of size ", it->GetLength (), " can't be sent. Dropped");
				}
		}
	}
		...
		switch (m_Server.GetPeerTestParticipant (nonce))
		{
			...
			case ePeerTestParticipantAlice1:
			{
				if (m_Server.GetPeerTestSession (nonce) == shared_from_this ()) // Alice-Bob
				{
					...
					if (dotnet::context.GetStatus () == eRouterStatusTesting) // still not OK
						dotnet::context.SetStatus (eRouterStatusFirewalled);
				}
				else
				{
	...
	// sendAddress is false if message comes from Alice
	{
		...
		if (toAddress)
		{
			...
			auto addr = dotnet::context.GetRouterInfo ().GetSSUAddress ();
			if (addr)
	...
	void SSUSession::SendPeerTest ()
	{
		...
		auto address = dotnet::context.GetRouterInfo ().GetSSUAddress (dotnet::context.SupportsV4 ());
		if (!address)
		{
			LogPrint (eLogInfo, "SSU is not supported. Can't send peer test");
			return;
	...
	{
		m_NumSentBytes += size;
		dotnet::transport::transports.UpdateSentBytes (size);
