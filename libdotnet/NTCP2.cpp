...
namespace transport
{
	...
	void NTCP2Establisher::KeyDerivationFunction1 (const uint8_t * pub, dotnet::crypto::X25519Keys& priv, const uint8_t * rs, const uint8_t * epub)
	{
	...
	{
		KeyDerivationFunction1 (GetRemotePub (), dotnet::context.GetStaticKeys (), dotnet::context.GetNTCP2StaticPublicKey (), GetRemotePub ()); 
	}
	...
	{
		uint8_t inputKeyMaterial[32];
		dotnet::context.GetStaticKeys ().Agree (GetRemotePub (), inputKeyMaterial);		 
		MixKey (inputKeyMaterial);
	}
	...
	void NTCP2Establisher::CreateSessionRequestMessage ()
	{
		...
		// m3p2Len	
		auto bufLen = dotnet::context.GetRouterInfo ().GetBufferLen ();
		...
		m3p2[3] = 0; // flag 	
		memcpy (m3p2 + 4, dotnet::context.GetRouterInfo ().GetBuffer (), bufLen); // TODO: own RI should be protected by mutex
		// 2 bytes reserved
		htobe32buf (options + 8, dotnet::util::GetSecondsSinceEpoch ()); // tsA
		// 4 bytes reserved
		// sign and encrypt options, use m_H as AD			
		uint8_t nonce[12];
		memset (nonce, 0, 12); // set nonce to zero
		dotnet::crypto::AEADChaCha20Poly1305 (options, 16, m_H, 32, m_K, nonce, m_SessionRequestBuffer + 32, 32, true); // encrypt
	}
	...
	void NTCP2Establisher::CreateSessionCreatedMessage ()
	{
		...
		// encrypt Y
		dotnet::crypto::CBCEncryption encryption;
		encryption.SetKey (dotnet::context.GetIdentHash ());
		encryption.SetIV (m_IV);
		...
		htobe32buf (options + 8, dotnet::util::GetSecondsSinceEpoch ()); // tsB
		// sign and encrypt options, use m_H as AD			
		uint8_t nonce[12];
		memset (nonce, 0, 12); // set nonce to zero
		dotnet::crypto::AEADChaCha20Poly1305 (options, 16, m_H, 32, m_K, nonce, m_SessionCreatedBuffer + 32, 32, true); // encrypt
	...
	void NTCP2Establisher::CreateSessionConfirmedMessagePart1 (const uint8_t * nonce)
	{
		...
		// part1 48 bytes  
		dotnet::crypto::AEADChaCha20Poly1305 (dotnet::context.GetNTCP2StaticPublicKey (), 32, m_H, 32, m_K, nonce, m_SessionConfirmedBuffer, 48, true); // encrypt
	}
	...
	void NTCP2Establisher::CreateSessionConfirmedMessagePart2 (const uint8_t * nonce)
	{
		...
		KDF3Alice (); 
		uint8_t * m3p2 = m_SessionConfirmedBuffer + 48;
		dotnet::crypto::AEADChaCha20Poly1305 (m3p2, m3p2Len - 16, m_H, 32, m_K, nonce, m3p2, m3p2Len, true); // encrypt 
		// update h again
	...
	bool NTCP2Establisher::ProcessSessionRequestMessage (uint16_t& paddingLen)
	{
		...
		decryption.SetKey (dotnet::context.GetIdentHash ());
		decryption.SetIV (dotnet::context.GetNTCP2IV ());
		...
		memset (nonce, 0, 12); // set nonce to zero
		if (dotnet::crypto::AEADChaCha20Poly1305 (m_SessionRequestBuffer + 32, 16, m_H, 32, m_K, nonce, options, 16, false)) // decrypt
		{
			// options
	...
	bool NTCP2Establisher::ProcessSessionCreatedMessage (uint16_t& paddingLen)
	{
		...
		uint8_t nonce[12];
		memset (nonce, 0, 12); // set nonce to zero
		if (dotnet::crypto::AEADChaCha20Poly1305 (m_SessionCreatedBuffer + 32, 16, m_H, 32, m_K, nonce, payload, 16, false)) // decrypt
		{
			// options		
	...
	bool NTCP2Establisher::ProcessSessionConfirmedMessagePart1 (const uint8_t * nonce)
	{
		...
		if (!dotnet::crypto::AEADChaCha20Poly1305 (m_SessionConfirmedBuffer, 32, m_H, 32, m_K, nonce, m_RemoteStaticKey, 32, false)) // decrypt S
		{
	...
	bool NTCP2Establisher::ProcessSessionConfirmedMessagePart2 (const uint8_t * nonce, uint8_t * m3p2Buf)
	{
		...
		KDF3Bob (); 
		if (dotnet::crypto::AEADChaCha20Poly1305 (m_SessionConfirmedBuffer + 48, m3p2Len - 16, m_H, 32, m_K, nonce, m3p2Buf, m3p2Len - 16, false)) // decrypt
		{
	...
	NTCP2Session::NTCP2Session (NTCP2Server& server, std::shared_ptr<const dotnet::data::RouterInfo> in_RemoteRouter):
		...
		else
		{
			...
			if (m_Establisher->ProcessSessionConfirmedMessagePart1 (nonce))
			{
				...
				if (m_Establisher->ProcessSessionConfirmedMessagePart2 (nonce, buf.data ()))
				{
					...
					// TODO: check flag
					dotnet::data::RouterInfo ri (buf.data () + 4, size - 1); // 1 byte block type + 2 bytes size + 1 byte flag
					if (ri.IsUnreachable ())
					{
					...
					if (dotnet::util::GetMillisecondsSinceEpoch () > ri.GetTimestamp () + dotnet::data::NETDB_MIN_EXPIRATION_TIMEOUT*1000LL) // 90 minutes
					{
					...
					dotnet::data::netdb.PostDNNPMsg (CreateDNNPMessage (eDNNPDummyMsg, buf.data () + 3, size)); // TODO: should insert ri and not parse it twice
					// TODO: process options
						
					// ready to communicate	
					auto existing = dotnet::data::netdb.FindRouter (ri.GetRouterIdentity ()->GetIdentHash ()); // check if exists already
...
#else
			dotnet::crypto::Siphash<8> (m_ReceiveIV.buf, m_ReceiveIV.buf, 8, m_ReceiveSipKey);
#endif
	...
	void NTCP2Session::HandleReceived (const boost::system::error_code& ecode, std::size_t bytes_transferred)
	{
		...
		else
		{
			...
			dotnet::transport::transports.UpdateReceivedBytes (bytes_transferred);
			uint8_t nonce[12];
			CreateNonce (m_ReceiveSequenceNumber, nonce); m_ReceiveSequenceNumber++;
			if (dotnet::crypto::AEADChaCha20Poly1305 (m_NextReceivedBuffer, m_NextReceivedLen-16, nullptr, 0, m_ReceiveKey, nonce, m_NextReceivedBuffer, m_NextReceivedLen, false))
			{	
		...
		while (offset < len)
		{
			...
			switch (blk)
			{
				...
				case eNTCP2BlkRouterInfo:
				{
					...
					dotnet::data::netdb.PostDNNPMsg (CreateDNNPMessage (eDNNPDummyMsg, frame + offset, size));
					break;
				}
				case eNTCP2BlkDNNPMessage:
				{
					LogPrint (eLogDebug, "NTCP2: DNNP");
					if (size > DNNP_MAX_MESSAGE_SIZE)
					{
						LogPrint (eLogError, "NTCP2: DNNP block is too long ", size);
						break;
					}
					auto nextMsg = NewDNNPMessage (size);
					nextMsg->Align (12); // for possible tunnel msg
					nextMsg->len = nextMsg->offset + size + 7; // 7 more bytes for full DNNP header
...
#else
		dotnet::crypto::Siphash<8> (m_SendIV.buf, m_SendIV.buf, 8, m_SendSipKey);
#endif
	...
	void NTCP2Session::SendDNNPMsgs (std::vector<std::shared_ptr<DNNPMessage> >& msgs)
	{
		...
		std::shared_ptr<DNNPMessage> first;
		...
		{	
			...
			// block header
			buf -= 3; 
			buf[0] = eNTCP2BlkDNNPMessage; // blk
		...
		dotnet::crypto::AEADChaCha20Poly1305Encrypt (encryptBufs, m_SendKey, nonce, macBuf); // encrypt buffers
		...
		boost::asio::async_write (m_Socket, bufs, boost::asio::transfer_all (),
			std::bind(&NTCP2Session::HandleDNNPMsgsSent, shared_from_this (), std::placeholders::_1, std::placeholders::_2, msgs));
	}	

	void NTCP2Session::HandleDNNPMsgsSent (const boost::system::error_code& ecode, std::size_t bytes_transferred, std::vector<std::shared_ptr<DNNPMessage> > msgs)
	{
	...
	void NTCP2Session::EncryptAndSendNextBuffer (size_t payloadLen)
	{
		...
		uint8_t nonce[12];
		CreateNonce (m_SendSequenceNumber, nonce); m_SendSequenceNumber++;
		dotnet::crypto::AEADChaCha20Poly1305Encrypt ({ {m_NextSendBuffer + 2, payloadLen} }, m_SendKey, nonce, m_NextSendBuffer + payloadLen + 2);	
	...
	void NTCP2Session::HandleNextFrameSent (const boost::system::error_code& ecode, std::size_t bytes_transferred)
	{
		...
		{	
			...
			dotnet::transport::transports.UpdateSentBytes (bytes_transferred);
	...
	void NTCP2Session::SendQueue ()
	{
		...
		{
			std::vector<std::shared_ptr<DNNPMessage> > msgs;
			size_t s = 0;
			while (!m_SendQueue.empty ())
			{
				...
				{
					LogPrint (eLogError, "NTCP2: DNNP message of size ", len, " can't be sent. Dropped");
					m_SendQueue.pop_front ();
				}
				else
					break;
	...
	void NTCP2Session::SendRouterInfo ()
	{
		...
		auto riLen = dotnet::context.GetRouterInfo ().GetBufferLen ();
		...
		memcpy (m_NextSendBuffer + 6, dotnet::context.GetRouterInfo ().GetBuffer (), riLen);
		// padding block
	...
	void NTCP2Session::SendDNNPMessages (const std::vector<std::shared_ptr<DNNPMessage> >& msgs)
	{
		m_Server.GetService ().post (std::bind (&NTCP2Session::PostDNNPMessages, shared_from_this (), msgs));
	}

	void NTCP2Session::PostDNNPMessages (std::vector<std::shared_ptr<DNNPMessage> > msgs)
	{
	...
	std::shared_ptr<NTCP2Session> NTCP2Server::FindNTCP2Session (const dotnet::data::IdentHash& ident)
	{
		auto it = m_NTCP2Sessions.find (ident);
		if (it != m_NTCP2Sessions.end ())
			return it->second;
				...
				if (this->AddNTCP2Session (conn))
				{
					...
					timer->async_wait ([conn, timeout](const boost::system::error_code& ecode) 
					{
						if (ecode != boost::asio::error::operation_aborted)
						{
							...
							//dotnet::data::netdb.SetUnreachable (conn->GetRemoteIdentity ()->GetIdentHash (), true);
							conn->Terminate ();
						}
					});
