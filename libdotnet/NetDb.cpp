...
namespace data
{
	...
	void NetDb::Start ()
	{
		m_Storage.SetPlace(dotnet::fs::GetDataDir());
		m_Storage.Init(dotnet::data::GetBase64SubstitutionTable(), 64);
		InitProfilesStorage ();
		m_Families.LoadCertificates ();
		Load ();

		uint16_t threshold; dotnet::config::GetOption("reseed.threshold", threshold);
		if (m_RouterInfos.size () < threshold) // reseed if # of router less than threshold
			Reseed ();

		dotnet::config::GetOption("persist.profiles", m_PersistProfiles);
	...
	void NetDb::Run ()
	{
		...
		while (m_IsRunning)
		{
			try
			{
				...
				if (ts - lastDestinationCleanup >= dotnet::garlic::INCOMING_TAGS_EXPIRATION_TIMEOUT)
				{
					dotnet::context.CleanupDestination ();
					lastDestinationCleanup = ts;
				}

				if (ts - lastPublish >= NETDB_PUBLISH_INTERVAL) // update timestamp and publish
				{
					dotnet::context.UpdateTimestamp (ts);
	...
	bool NetDb::AddLeaseSet (const IdentHash& ident, const uint8_t * buf, int len)
	{
		...
		if (it != m_LeaseSets.end () && it->second->GetStoreType () == dotnet::data::NETDB_STORE_TYPE_LEASESET)
		{
	...
	void NetDb::ReseedFromFloodfill(const RouterInfo & ri, int numRouters, int numFloodfills)
	{
		...
		std::vector<std::shared_ptr<dotnet::DNNPMessage> > requests;

		dotnet::data::IdentHash ourIdent = dotnet::context.GetIdentHash();
		dotnet::data::IdentHash ih = ri.GetIdentHash();
		dotnet::data::IdentHash randomIdent;

		// make floodfill lookups
		while(numFloodfills > 0) {
			randomIdent.Randomize();
			auto msg = dotnet::CreateRouterInfoDatabaseLookupMsg(randomIdent, ourIdent, 0, false);
		...
		while(numRouters > 0) {
			randomIdent.Randomize();
			auto msg = dotnet::CreateRouterInfoDatabaseLookupMsg(randomIdent, ourIdent, 0, true);
			requests.push_back(msg);
			numRouters --;
		}

		// send them off
		dotnet::transport::transports.SendMessages(ih, requests);
	}
	...
	void NetDb::VisitStoredRouterInfos(RouterInfoVisitor v)
	{
		...
		{
			auto ri = std::make_shared<dotnet::data::RouterInfo>(filename);
				v(ri);
		});
	}
	...
	void NetDb::Load ()
	{
		...
		m_LastLoad = dotnet::util::GetSecondsSinceEpoch();
	...
	void NetDb::SaveUpdated ()
	{
		...
		bool checkForExpiration = total > NETDB_MIN_ROUTERS && ts > (dotnet::context.GetStartupTime () + 600)*1000LL; // 10 minutes
		if (checkForExpiration && ts > (dotnet::context.GetStartupTime () + 3600)*1000LL) // 1 hour
			expirationTimeout = dotnet::context.IsFloodfill () ? NETDB_FLOODFILL_EXPIRATION_TIMEOUT*1000LL :
	...
	void NetDb::HandleNTCP2RouterInfoMsg (std::shared_ptr<const DNNPMessage> m)
	{
	...
	void NetDb::HandleDatabaseStoreMsg (std::shared_ptr<const DNNPMessage> m)
	{
		...
		if (ident == dotnet::context.GetIdentHash ())
		{
		...
		{
			// flood updated
			auto floodMsg = NewDNNPShortMessage ();
			...
			if (floodMsg->len < floodMsg->maxLen)
			{
				...
				floodMsg->FillDNNPMessageHeader (eDNNPDatabaseStore);
	...
	void NetDb::HandleDatabaseSearchReplyMsg (std::shared_ptr<const DNNPMessage> msg)
	{
		const uint8_t * buf = msg->GetPayload ();
		char key[48];
		int l = dotnet::data::ByteStreamToBase64 (buf, 32, key, 48);
		key[l] = 0;
		int num = buf[32]; // num
		...
		for (int i = 0; i < num; i++)
		{
			...
			char peerHash[48];
			int l1 = dotnet::data::ByteStreamToBase64 (router, 32, peerHash, 48);
			peerHash[l1] = 0;
			LogPrint (eLogDebug, "NetDb: ", i, ": ", peerHash);

			auto r = FindRouter (router);
			if (!r || dotnet::util::GetMillisecondsSinceEpoch () > r->GetTimestamp () + 3600*1000LL)
			{
	...
	void NetDb::HandleDatabaseLookupMsg (std::shared_ptr<const DNNPMessage> msg)
	{
		const uint8_t * buf = msg->GetPayload ();
		IdentHash ident (buf);
		if (ident.IsZero ())
		...
		std::shared_ptr<DNNPMessage> replyMsg;
		...
		if (replyMsg)
		{
			if (replyTunnelID)
			{
				...
				if (flag & DATABASE_LOOKUP_ENCRYPTION_FLAG) // encrypted reply requested
				{
					...
					if (numTags)
					{
						const dotnet::garlic::SessionTag sessionTag(excluded + 33); // take first tag
						dotnet::garlic::GarlicRoutingSession garlic (sessionKey, sessionTag);
				...
				auto exploratoryPool = dotnet::tunnel::tunnels.GetExploratoryPool ();
				auto outbound = exploratoryPool ? exploratoryPool->GetNextOutboundTunnel () : nullptr;
				if (outbound)
					outbound->SendTunnelDataMsg (replyIdent, replyTunnelID, replyMsg);
				else
					transports.SendMessage (replyIdent, dotnet::CreateTunnelGatewayMsg (replyTunnelID, replyMsg));
			}
			else
	...
	void NetDb::Explore (int numDestinations)
	{
		...
		for (int i = 0; i < numDestinations; i++)
		{
			...
			if (floodfill)
			{
				if (dotnet::transport::transports.IsConnected (floodfill->GetIdentHash ()))
				...
				if (throughTunnels)
				{
					...
					dotnet::transport::transports.SendMessage (floodfill->GetIdentHash (), dest->CreateRequestMessage (floodfill->GetIdentHash ()));
			}
			else
	...
	void NetDb::Publish ()
	{
		dotnet::context.UpdateStats (); // for floodfill
		std::set<IdentHash> excluded; // TODO: fill up later
		for (int i = 0; i < 2; i++)
		{
			auto floodfill = GetClosestFloodfill (dotnet::context.GetRouterInfo ().GetIdentHash (), excluded);
			if (floodfill)
			{
				uint32_t replyToken;
				RAND_bytes ((uint8_t *)&replyToken, 4);
				LogPrint (eLogInfo, "NetDb: Publishing our RouterInfo to ", dotnet::data::GetIdentHashAbbreviation(floodfill->GetIdentHash ()), ". reply token=", replyToken);
				transports.SendMessage (floodfill->GetIdentHash (), CreateDatabaseStoreMsg (dotnet::context.GetSharedRouterInfo (), replyToken));
				excluded.insert (floodfill->GetIdentHash ());
			}
		}
	}

	void NetDb::Flood (const IdentHash& ident, std::shared_ptr<DNNPMessage> floodMsg)
	{
		std::set<IdentHash> excluded;
		excluded.insert (dotnet::context.GetIdentHash ()); // don't flood to itself
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (floodfill)
			{
				...
				transports.SendMessage (h, CopyDNNPMessage(floodMsg));
				excluded.insert (h);
			}
			else
				break;
	...
	void NetDb::PostDNNPMsg (std::shared_ptr<const DNNPMessage> msg)
	{
		...
		const std::set<IdentHash>& excluded, bool closeThanUsOnly) const
	{
		...
		if (closeThanUsOnly)
			minMetric = destKey ^ dotnet::context.GetIdentHash ();
		else
			minMetric.SetMax ();
		std::unique_lock<std::mutex> l(m_FloodfillsMutex);
		...
		std::set<IdentHash>& excluded, bool closeThanUsOnly) const
	{
		...
		IdentHash destKey = CreateRoutingKey (destination);
		XORMetric ourMetric;
		if (closeThanUsOnly) ourMetric = destKey ^ dotnet::context.GetIdentHash ();
		{
			std::unique_lock<std::mutex> l(m_FloodfillsMutex);
			for (const auto& it: m_Floodfills)
			{
