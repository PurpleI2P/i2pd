...
namespace proxy
{
	...
	class SOCKSServer;
	class SOCKSHandler: public dotnet::client::DotNetServiceHandler, public std::enable_shared_from_this<SOCKSHandler>
	{
		private:
			enum state
			{
		...
		std::size_t m_upstream_response_len;
			...
			state m_state;
			const bool m_UseUpstreamProxy; // do we want to use the upstream proxy for non dotnet addresses?
	...
	void SOCKSHandler::SocksRequestSuccess()
	{
		...
		switch (m_socksv)
		{
			...
			case SOCKS5:
				...
				auto s = dotnet::client::context.GetAddressBook().ToAddress(GetOwner()->GetLocalDestination()->GetIdentHash());
	...
	void SOCKSHandler::HandleSockRecv(const boost::system::error_code & ecode, std::size_t len)
	{
		...
		if (HandleData(m_sock_buff, len))
		{
			if (m_state == READY)
			{
				...
				// does it end with .dotnet?
				if ( addr.rfind(".dotnet") == addrlen - 4) {
					// yes it does, make an dotnet session
	...
	void SOCKSHandler::SentSocksDone(const boost::system::error_code & ecode)
	{
		...
		{
			if (Kill()) return;
			LogPrint (eLogInfo, "SOCKS: new DotNetTunnel connection");
			auto connection = std::make_shared<dotnet::client::DotNetTunnelConnection>(GetOwner(), m_sock, m_stream);
			GetOwner()->AddHandler (connection);
			connection->DOTNETConnect (m_remaining_data,m_remaining_data_len);
			Done(shared_from_this());
		}
		else
		{
	...
	void SOCKSHandler::HandleStreamRequestComplete (std::shared_ptr<dotnet::stream::Stream> stream)
	{
		if (stream)
		{
			m_stream = stream;
	...
	void SOCKSHandler::SocksUpstreamSuccess()
	{
		...
		auto forwarder = std::make_shared<dotnet::client::TCPIPPipe>(GetOwner(), m_sock, m_upstreamSock);
		m_upstreamSock = nullptr;
		m_sock = nullptr;
	...
	SOCKSServer::SOCKSServer(const std::string& name, const std::string& address, int port,
		...
		TCPIPAcceptor (address, port, localDestination ? localDestination : dotnet::client::context.GetSharedLocalDestination ()), m_Name (name)
	{
	...
	std::shared_ptr<dotnet::client::DotNetServiceHandler> SOCKSServer::CreateHandler(std::shared_ptr<boost::asio::ip::tcp::socket> socket)
	{
