...
namespace client
{
	...
	class BOBDotNetTunnel: public DotNetService
	{
		public:

			BOBDotNetTunnel (std::shared_ptr<ClientDestination> localDestination):
	...
	class BOBDOTNETInboundTunnel: public BOBDotNetTunnel
	{
		...
		public:
			...
			BOBDOTNETInboundTunnel (const boost::asio::ip::tcp::endpoint& ep, std::shared_ptr<ClientDestination> localDestination);
			~BOBDOTNETInboundTunnel ();
		...
		private:
			...
			void HandleDestinationRequestComplete (std::shared_ptr<dotnet::data::LeaseSet> leaseSet, std::shared_ptr<AddressReceiver> receiver);

			void CreateConnection (std::shared_ptr<AddressReceiver> receiver, std::shared_ptr<const dotnet::data::LeaseSet> leaseSet);
	...
	class BOBDOTNETOutboundTunnel: public BOBDotNetTunnel
	{
		public:

			 BOBDOTNETOutboundTunnel (const std::string& address, int port, std::shared_ptr<ClientDestination> localDestination, bool quiet);
	...
	class BOBDestination
	{
		public:
			...
			const dotnet::data::PrivateKeys& GetKeys () const { return m_LocalDestination->GetPrivateKeys (); };
			std::shared_ptr<ClientDestination> GetLocalDestination () const { return m_LocalDestination; };

		private:

			std::shared_ptr<ClientDestination> m_LocalDestination;
			BOBDOTNETOutboundTunnel * m_OutboundTunnel;
			BOBDOTNETInboundTunnel * m_InboundTunnel;
	};
