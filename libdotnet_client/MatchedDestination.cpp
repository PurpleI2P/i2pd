...
namespace client
{
	MatchedTunnelDestination::MatchedTunnelDestination(const dotnet::data::PrivateKeys & keys, const std::string & remoteName, const std::map<std::string, std::string> * params)
	...
	{
		auto addr = dotnet::client::context.GetAddressBook().GetAddress (m_RemoteName);	
	...
	void MatchedTunnelDestination::HandleFoundCurrentLeaseSet(std::shared_ptr<const dotnet::data::LeaseSet> ls)
	{
		if(ls)
		{
	...
	bool MatchedTunnelDestination::SelectPeers(dotnet::tunnel::Path & path, int hops, bool inbound)
	{
		auto pool = GetTunnelPool();
		if(!dotnet::tunnel::StandardSelectPeers(path, hops, inbound, std::bind(&dotnet::tunnel::TunnelPool::SelectNextHop, pool, std::placeholders::_1)))
			return false;
		...
		if(!inbound && m_RemoteLeaseSet)
		{
			...
			if(m_RemoteLeaseSet && !m_RemoteLeaseSet->IsExpired())
			{
				...
				// pick lease
				std::shared_ptr<dotnet::data::RouterInfo> obep;
				while(!obep && leases.size() > 0) {
					auto idx = rand() % leases.size();
					auto lease = leases[idx];
					obep = dotnet::data::netdb.FindRouter(lease->tunnelGateway);
	...
	bool MatchedTunnelDestination::OnBuildResult(const dotnet::tunnel::Path & path, bool inbound, dotnet::tunnel::TunnelBuildResult result)
	{
		return true;
	}
}
