...
namespace client
{
	...
	void SAMSocket::ProcessSessionCreate (char * buf, size_t len)
	{
		...
		if (style == SAM_VALUE_DATAGRAM && params.find(SAM_VALUE_HOST) != params.end() && params.find(SAM_VALUE_PORT) != params.end())
		{
			...
			{
				// not an ip address
				SendDOTNETError("Invalid IP Address in HOST");
				return;
			}
		...
		if (session)
		{
			...
			if (style == SAM_VALUE_DATAGRAM)
			{
				...
				dest->SetReceiver (std::bind (&SAMSocket::HandleDOTNETatagramReceive, shared_from_this (),
	...
	void SAMSocket::SendSessionCreateReplyOk ()
	{
		...
		if (session)
		{
			...
			size_t l1 = dotnet::data::ByteStreamToBase64 (buf, l, priv, 1024);
			priv[l1] = 0;
#ifdef _MSC_VER
	...
	void SAMSocket::ProcessStreamConnect (char * buf, size_t len, size_t rem)
	{
		...
		if (session)
		{
			...
			auto dest = std::make_shared<dotnet::data::IdentityEx> ();
	...
	void SAMSocket::Connect (std::shared_ptr<const dotnet::data::LeaseSet> remote)
	{
		auto session = m_Owner.FindSession(m_ID);
		if(session)
		{
	...
	void SAMSocket::HandleConnectLeaseSetRequestComplete (std::shared_ptr<dotnet::data::LeaseSet> leaseSet)
	{
		if (leaseSet)
			Connect (leaseSet);
		else
	...
	void SAMSocket::ProcessStreamAccept (char * buf, size_t len)
	{
		...
		if (session)
		{
			...
			{
				m_IsAccepting = true;	
				session->localDestination->AcceptOnce (std::bind (&SAMSocket::HandleDOTNETAccept, shared_from_this (), std::placeholders::_1));
			}
			SendMessageReply (SAM_STREAM_STATUS_OK, strlen(SAM_STREAM_STATUS_OK), false);
		}
		else
	...
	void SAMSocket::ProcessDestGenerate (char * buf, size_t len)
	{
		...
		auto keys = dotnet::data::PrivateKeys::CreateRandomKeys (signatureType, cryptoType);
#ifdef _MSC_VER
	...
	void SAMSocket::ProcessNamingLookup (char * buf, size_t len)
	{
		...
		std::shared_ptr<const dotnet::data::IdentityEx> identity;
	...
	void SAMSocket::SendDOTNETError(const std::string & msg)
	{
		LogPrint (eLogError, "SAM: dotnet error ", msg);
#ifdef _MSC_VER
		size_t len = sprintf_s (m_Buffer, SAM_SOCKET_BUFFER_SIZE, SAM_SESSION_STATUS_DOTNET_ERROR, msg.c_str());
#else
		size_t len = snprintf (m_Buffer, SAM_SOCKET_BUFFER_SIZE, SAM_SESSION_STATUS_DOTNET_ERROR, msg.c_str());
#endif
		SendMessageReply (m_Buffer, len, true);
	}

	void SAMSocket::HandleNamingLookupLeaseSetRequestComplete (std::shared_ptr<dotnet::data::LeaseSet> leaseSet, std::string name)
	{
		if (leaseSet)
		{
	...
	void SAMSocket::SendNamingLookupReply (std::shared_ptr<const dotnet::data::IdentityEx> identity)
	{
		...
		if (m_Stream)
		{
					 ...
					 m_Stream->GetStatus () == dotnet::stream::eStreamStatusOpen) // regular
			{
				m_Stream->AsyncReceive (boost::asio::buffer (m_StreamBuffer, SAM_SOCKET_BUFFER_SIZE),
						std::bind (&SAMSocket::HandleDOTNETReceive, shared_from_this(),
						std::placeholders::_1, std::placeholders::_2),
			...
			else // closed by peer
			{
				...
				{
					WriteDOTNETataImmediate(buff, len);
				}
				else // no more data
				{
					delete [] buff;
	...
	void SAMSocket::WriteDOTNETataImmediate(uint8_t * buff, size_t sz)
	{
		boost::asio::async_write (
			m_Socket,
			boost::asio::buffer (buff, sz),
			boost::asio::transfer_all(),
			std::bind (&SAMSocket::HandleWriteDOTNETataImmediate, shared_from_this (), std::placeholders::_1, buff)); // postpone termination
	}

	void SAMSocket::HandleWriteDOTNETataImmediate(const boost::system::error_code & ec, uint8_t * buff)
	{
		delete [] buff;
	}
	
	void SAMSocket::WriteDOTNETata(size_t sz)
	{
		boost::asio::async_write (
			m_Socket,
			boost::asio::buffer (m_StreamBuffer, sz),
			boost::asio::transfer_all(),
			std::bind(&SAMSocket::HandleWriteDOTNETata, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
	}
	
	void SAMSocket::HandleDOTNETReceive (const boost::system::error_code& ecode, std::size_t bytes_transferred)
	{
		if (ecode)
		{
			...
			if (ecode != boost::asio::error::operation_aborted)
			{
				...
				{
					WriteDOTNETata(bytes_transferred);
				}
				else
				{
	...
	void SAMSocket::HandleWriteDOTNETata (const boost::system::error_code& ecode, size_t bytes_transferred)
	{
		if (ecode)
		{
	...
	void SAMSocket::HandleDOTNETAccept (std::shared_ptr<dotnet::stream::Stream> stream)
	{
		if (stream)
		{
			...
			if (session)
			{
				...
				for (auto & it: m_Owner.ListSockets (m_ID))
					if (it->m_SocketType == eSAMSocketTypeAcceptor)
					{
						...
						session->localDestination->AcceptOnce (std::bind (&SAMSocket::HandleDOTNETAccept, it, std::placeholders::_1));
						break;
					}
			}
			if (!m_IsSilent)
			{
				...
				const size_t l1 = dotnet::data::ByteStreamToBase64 (ident, l, (char *)m_StreamBuffer, SAM_SOCKET_BUFFER_SIZE);
				delete[] ident;
				m_StreamBuffer[l1] = '\n';
				HandleDOTNETReceive (boost::system::error_code (), l1 +1); // we send identity like it has been received from stream
			}
			else
				DOTNETReceive ();
		}
	...
	{
		dotnet::client::context.DeleteLocalDestination (localDestination);
	}
	...
	SAMBridge::SAMBridge (const std::string& address, int port):
		...
		m_SignatureTypes
		{
			{"DSA_SHA1", dotnet::data::SIGNING_KEY_TYPE_DSA_SHA1},
			{"ECDSA_SHA256_P256", dotnet::data::SIGNING_KEY_TYPE_ECDSA_SHA256_P256},
			{"ECDSA_SHA256_P384", dotnet::data::SIGNING_KEY_TYPE_ECDSA_SHA384_P384},
			{"ECDSA_SHA256_P521", dotnet::data::SIGNING_KEY_TYPE_ECDSA_SHA512_P521},
			{"EdDSA_SHA512_Ed25519", dotnet::data::SIGNING_KEY_TYPE_EDDSA_SHA512_ED25519},
			{"GOST_GOSTR3411256_GOSTR3410CRYPTOPROA", dotnet::data::SIGNING_KEY_TYPE_GOSTR3410_CRYPTO_PRO_A_GOSTR3411_256},
			{"GOST_GOSTR3411512_GOSTR3410TC26A512", dotnet::data::SIGNING_KEY_TYPE_GOSTR3410_TC26_A_512_GOSTR3411_512}
		}
	{
	}
		...
		const std::map<std::string, std::string> * params)
	{
		...
		if (destination != "")
		{
			...
			localDestination = dotnet::client::context.CreateNewLocalDestination (keys, true, params);
		}
		else // transient
		{
			// extract signature type
			...
			localDestination = dotnet::client::context.CreateNewLocalDestination (true, signatureType, cryptoType, params);
		}
		if (localDestination)
		{
	...
	void SAMBridge::ReceiveDatagram ()
	{
		m_DatagramSocket.async_receive_from (
			boost::asio::buffer (m_DatagramReceiveBuffer, dotnet::datagram::MAX_DATAGRAM_SIZE),
			m_SenderEndpoint,
	...
	bool SAMBridge::ResolveSignatureType (const std::string& name, dotnet::data::SigningKeyType& type) const
	{
		try
		{	
			type = std::stoi (name);
