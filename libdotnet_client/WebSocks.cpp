...
namespace client
{
	...
	typedef std::function<void(std::shared_ptr<dotnet::stream::Stream>)> StreamConnectFunc;


	struct IWebSocksConn : public DotNetServiceHandler
	{
		IWebSocksConn(DotNetService * parent) : DotNetServiceHandler(parent) {}
	...
	public:
		...
		{
			auto & addressbook = dotnet::client::context.GetAddressBook();
		...
		dotnet::datagram::DatagramDestination * GetDatagramDest() const
		{
	...
	struct WebSocksConn : public IWebSocksConn , public std::enable_shared_from_this<WebSocksConn>
	{
		...
		typedef std::shared_ptr<ClientDestination> Destination_t;
		typedef std::shared_ptr<dotnet::stream::StreamingDestination> StreamDest_t;
		typedef std::shared_ptr<dotnet::stream::Stream> Stream_t;
		...
		bool m_IsDatagram;
		dotnet::datagram::DatagramDestination * m_Datagram;
		...
		void HandleDatagram(const dotnet::data::IdentityEx& from, uint16_t fromPort, uint16_t toPort, const uint8_t * buf, size_t len)
		{
