...
namespace client
{
	...
	{
		private:
			dotnet::fs::HashedStorage storage;
			std::string etagsPath, indexPath, localPath;

		public:
			AddressBookFilesystemStorage (): storage("addressbook", "b", "", "b32") 
			{
				dotnet::config::GetOption("persist.addressbook", m_IsPersist);
			}
			std::shared_ptr<const dotnet::data::IdentityEx> GetAddress (const dotnet::data::IdentHash& ident) const;
			void AddAddress (std::shared_ptr<const dotnet::data::IdentityEx> address);
			void RemoveAddress (const dotnet::data::IdentHash& ident);
			...
			void SaveEtag (const dotnet::data::IdentHash& subsciption, const std::string& etag, const std::string& lastModified);
	...
	{
		storage.SetPlace(dotnet::fs::GetDataDir());
		// init storage
		if (storage.Init(dotnet::data::GetBase32SubstitutionTable(), 32))
		{
			// init ETags
			etagsPath = dotnet::fs::StorageRootPath (storage, "etags");
			if (!dotnet::fs::Exists (etagsPath))
				dotnet::fs::CreateDirectory (etagsPath);
			// init address files
			indexPath = dotnet::fs::StorageRootPath (storage, "addresses.csv");
			localPath = dotnet::fs::StorageRootPath (storage, "local.csv");
			return true;
		}
		return false;
	}

	std::shared_ptr<const dotnet::data::IdentityEx> AddressBookFilesystemStorage::GetAddress (const dotnet::data::IdentHash& ident) const
	{
		if (!m_IsPersist) 
		{
		...
		size_t len = f.tellg ();
		if (len < dotnet::data::DEFAULT_IDENTITY_SIZE) {
		...
		f.read((char *)buf, len);
		auto address = std::make_shared<dotnet::data::IdentityEx>(buf, len);
		delete[] buf;
		return address;
	}

	void AddressBookFilesystemStorage::AddAddress (std::shared_ptr<const dotnet::data::IdentityEx> address)
	{
	...
	void AddressBookFilesystemStorage::RemoveAddress (const dotnet::data::IdentHash& ident)
	{
		if (!m_IsPersist) return;	
	...
	void AddressBookFilesystemStorage::SaveEtag (const dotnet::data::IdentHash& subscription, const std::string& etag, const std::string& lastModified)
	{
		std::string fname = etagsPath + dotnet::fs::dirSep + subscription.ToBase32 () + ".txt";
	...
	bool AddressBookFilesystemStorage::GetEtag (const dotnet::data::IdentHash& subscription, std::string& etag, std::string& lastModified)
	{
		std::string fname = etagsPath + dotnet::fs::dirSep + subscription.ToBase32 () + ".txt";
...
//---------------------------------------------------------------------
	...
	Address::Address (const std::string& b32)
	{
		...
		else
		{
			...
			blindedPublicKey = std::make_shared<dotnet::data::BlindedPublicKey>(b32);
		}
	}	

	Address::Address (const dotnet::data::IdentHash& hash)
	{
		addressType = eAddressIndentHash;
		identHash = hash;	
	}
	...
	void AddressBook::InsertAddress (const std::string& address, const std::string& jump)
	{
		...
		{	
			// assume base64	
			auto ident = std::make_shared<dotnet::data::IdentityEx>();
	...
	void AddressBook::InsertFullAddress (std::shared_ptr<const dotnet::data::IdentityEx> address)
	{
		m_Storage->AddAddress (address);
	}

	std::shared_ptr<const dotnet::data::IdentityEx> AddressBook::GetFullAddress (const std::string& address)
	{
	...
	void AddressBook::LoadHosts ()
	{
		...
		// then try hosts.txt
		std::ifstream f (dotnet::fs::DataDirPath("hosts.txt"), std::ifstream::in); // in text mode
		if (f.is_open ())
		{
	...
	void AddressBook::LoadSubscriptions ()
	{
		...
		{
			std::ifstream f (dotnet::fs::DataDirPath ("subscriptions.txt"), std::ifstream::in); // in text mode
			if (f.is_open ())
			{
				std::string s;
				while (!f.eof ())
			...
			else if (!dotnet::config::IsDefault("addressbook.subscriptions"))
                        {
                            // using config file items
                            std::string subscriptionURLs; dotnet::config::GetOption("addressbook.subscriptions", subscriptionURLs);
	...
	bool AddressBook::GetEtag (const dotnet::data::IdentHash& subscription, std::string& etag, std::string& lastModified)
	{
		if (m_Storage)
	...
	void AddressBook::DownloadComplete (bool success, const dotnet::data::IdentHash& subscription, const std::string& etag, const std::string& lastModified)
	{
		m_IsDownloading = false;
		m_NumRetries++;
			...
			if (!m_IsDownloading && dest->IsReady ())
			{
				if (!m_IsLoaded)
				{
					...
					LogPrint (eLogInfo, "Addressbook: trying to download it from default subscription.");
                                        std::string defaultSubURL; dotnet::config::GetOption("addressbook.defaulturl", defaultSubURL);
	...
	void AddressBook::HandleLookupResponse (const dotnet::data::IdentityEx& from, uint16_t fromPort, uint16_t toPort, const uint8_t * buf, size_t len)
	{
		if (len < 44)
		{
		...
		{
			// TODO: verify from
			dotnet::data::IdentHash hash(buf + 8);
			if (!hash.IsZero ())
	...
	bool AddressBookSubscription::MakeRequest ()
	{
		...
		auto leaseSet = dotnet::client::context.GetSharedLocalDestination ()->FindLeaseSet (m_Ident);
		if (!leaseSet)
		{
			std::unique_lock<std::mutex> l(newDataReceivedMutex);
			dotnet::client::context.GetSharedLocalDestination ()->RequestDestination (m_Ident,
				[&newDataReceived, &leaseSet, &newDataReceivedMutex](std::shared_ptr<dotnet::data::LeaseSet> ls)
				{
					leaseSet = ls;
			...
			if (newDataReceived.wait_for (l, std::chrono::seconds (SUBSCRIPTION_REQUEST_TIMEOUT)) == std::cv_status::timeout)
			{
				...
				dotnet::client::context.GetSharedLocalDestination ()->CancelDestinationRequest (m_Ident, false); // don't notify, because we know it already
				return false;
			}
		}
		if (!leaseSet) {
		...
		req.AddHeader("X-Accept-Encoding", "x-dotnet-gzip;q=1.0, identity;q=0.5, deflate;q=0, gzip;q=0, *;q=0");
		...
		auto stream = dotnet::client::context.GetSharedLocalDestination ()->CreateStream (leaseSet, dest_port);
		...
		if (res.is_chunked())
		{
			...
			dotnet::http::MergeChunkedResponse (in, out);
			response = out.str();
		}
		else if (res.is_gzipped())
		{
			std::stringstream out;
			dotnet::data::GzipInflator inflator;
	...
	void AddressResolver::HandleRequest (const dotnet::data::IdentityEx& from, uint16_t fromPort, uint16_t toPort, const uint8_t * buf, size_t len)
	{
	...
	void AddressResolver::AddAddress (const std::string& name, const dotnet::data::IdentHash& ident)
	{
