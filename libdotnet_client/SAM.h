...
namespace client
{
	...
	const char SAM_HANDSHAKE_DOTNET_ERROR[] = "HELLO REPLY RESULT=DOTNET_ERROR\n";
	...
	const char SAM_SESSION_STATUS_DOTNET_ERROR[] = "SESSION STATUS RESULT=DOTNET_ERROR MESSAGE=%s\n";
	...
	const char SAM_STREAM_STATUS_DOTNET_ERROR[] = "STREAM STATUS RESULT=DOTNET_ERROR\n";
	const char SAM_STREAM_ACCEPT[] = "STREAM ACCEPT";
	const char SAM_DATAGRAM_SEND[] = "DATAGRAM SEND";
	const char SAM_DEST_GENERATE[] = "DEST GENERATE";
	const char SAM_DEST_REPLY[] = "DEST REPLY PUB=%s PRIV=%s\n";
	const char SAM_DEST_REPLY_DOTNET_ERROR[] = "DEST REPLY RESULT=DOTNET_ERROR\n";
	...
	class SAMSocket: public std::enable_shared_from_this<SAMSocket>
	{
		public:
		 ...
		 private:
			...
			void DOTNETReceive ();
			void HandleDOTNETReceive (const boost::system::error_code& ecode, std::size_t bytes_transferred);
			void HandleDOTNETAccept (std::shared_ptr<dotnet::stream::Stream> stream);
			void HandleWriteDOTNETata (const boost::system::error_code& ecode, size_t sz);
			void HandleDOTNETatagramReceive (const dotnet::data::IdentityEx& from, uint16_t fromPort, uint16_t toPort, const uint8_t * buf, size_t len);
			...
			void SendDOTNETError(const std::string & msg);
			size_t ProcessDatagramSend (char * buf, size_t len, const char * data); // from SAM 1.0
			void ExtractParams (char * buf, std::map<std::string, std::string>& params);

			void Connect (std::shared_ptr<const dotnet::data::LeaseSet> remote);
			void HandleConnectLeaseSetRequestComplete (std::shared_ptr<dotnet::data::LeaseSet> leaseSet);
			void SendNamingLookupReply (std::shared_ptr<const dotnet::data::IdentityEx> identity);
			void HandleNamingLookupLeaseSetRequestComplete (std::shared_ptr<dotnet::data::LeaseSet> leaseSet, std::string name);
			void HandleSessionReadinessCheckTimer (const boost::system::error_code& ecode);
			void SendSessionCreateReplyOk ();

			void WriteDOTNETata(size_t sz);
			void WriteDOTNETataImmediate(uint8_t * ptr, size_t sz);

			void HandleWriteDOTNETataImmediate(const boost::system::error_code & ec, uint8_t * buff);
	...
	class SAMBridge
	{
		public:
			...
			void RemoveSocket(const std::shared_ptr<SAMSocket> & socket);
			 ...
			 bool ResolveSignatureType (const std::string& name, dotnet::data::SigningKeyType& type) const;
		...
		private:
			...
			uint8_t m_DatagramReceiveBuffer[dotnet::datagram::MAX_DATAGRAM_SIZE+1];
			std::map<std::string, dotnet::data::SigningKeyType> m_SignatureTypes;
