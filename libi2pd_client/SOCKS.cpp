...
namespace i2p {
    namespace proxy {
        ...
        private:
            ...
            GenerateSOCKS5Response(errTypes error, addrTypes type, const address &addr, uint16_t port);
        ...
        boost::asio::const_buffers_1
        SOCKSHandler::GenerateSOCKS4Response(SOCKSHandler::errTypes error, uint32_t ip, uint16_t port) {
        ...
        boost::asio::const_buffers_1
        SOCKSHandler::GenerateSOCKS5Response(SOCKSHandler::errTypes error, SOCKSHandler::addrTypes type,
        ...
        boost::asio::const_buffers_1 SOCKSHandler::GenerateUpstreamRequest() {
            ...
            if (m_address.dns.size <= max_socks_hostname_size - (upstreamRequestSize + 1)) {
                ...
                LogPrint(eLogError,
                         "SOCKS: BUG!!! m_addr.dns.sizs > max_socks_hostname - ( upstreamRequestSize + 1 ) )");
            }
        ...
        bool SOCKSHandler::Socks5ChooseAuth() {
            ...
            if (m_authchosen == AUTH_UNACCEPTABLE) {
                ...
                boost::asio::async_write(*m_sock, response,
                                         std::bind(&SOCKSHandler::SentSocksFailed, shared_from_this(),
                                                   std::placeholders::_1));
                return false;
            } else {
                LogPrint(eLogDebug, "SOCKS: v5 choosing authentication method: ", m_authchosen);
                boost::asio::async_write(*m_sock, response,
                                         std::bind(&SOCKSHandler::SentSocksResponse, shared_from_this(),
                                                   std::placeholders::_1));
                return true;
            }
        }
        ...
        void SOCKSHandler::SocksRequestSuccess() {
            ...
            boost::asio::async_write(*m_sock, response, std::bind(&SOCKSHandler::SentSocksDone, shared_from_this(),
                                                                  std::placeholders::_1));
        }
        ...
        void SOCKSHandler::AsyncUpstreamSockRead() {
            ...
            if (m_upstreamSock) {
                m_upstreamSock->async_read_some(
                        boost::asio::buffer(m_upstream_response, SOCKS_UPSTREAM_SOCKS4A_REPLY_SIZE),
                        std::bind(&SOCKSHandler::HandleUpstreamSockRecv, shared_from_this(), std::placeholders::_1,
                                  std::placeholders::_2));
            } else {
        ...
        void
        SOCKSHandler::HandleUpstreamSockRecv(const boost::system::error_code &ecode, std::size_t bytes_transfered) {
            if (ecode) {
        ...
        void SOCKSHandler::HandleUpstreamResolved(const boost::system::error_code &ecode,
            ...
            if (ecode) {
                // error resolving
                LogPrint(eLogWarning, "SOCKS: Upstream proxy", m_UpstreamProxyAddress, " not resolved: ",
                         ecode.message());
                SocksRequestFailed(SOCKS5_NET_UNREACH);
                return;
            }
        ...
        SOCKSServer::CreateHandler(std::shared_ptr <boost::asio::ip::tcp::socket> socket) {
            return std::make_shared<SOCKSHandler>(this, socket, m_UpstreamProxyAddress, m_UpstreamProxyPort,
                                                  m_UseUpstreamProxy);
        }
