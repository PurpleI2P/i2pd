...
namespace i2p {
    namespace proxy {
        ...
        class HTTPReqHandler
                : public i2p::client::I2PServiceHandler, public std::enable_shared_from_this<HTTPReqHandler> {
        private:
            ...
            void HandleUpstreamProxyResolved(const boost::system::error_code &ecode,
                                             boost::asio::ip::tcp::resolver::iterator itr,
                                             ProxyResolvedHandler handler);
        ...
        void HTTPReqHandler::SendProxyError(std::string &content) {
            ...
            boost::asio::async_write(*m_sock, boost::asio::buffer(response), boost::asio::transfer_all(),
                                     std::bind(&HTTPReqHandler::SentHTTPFailed, shared_from_this(),
                                               std::placeholders::_1));
        }
        ...
        bool HTTPReqHandler::ExtractAddressHelper(i2p::http::URL &url, std::string &b64, bool &confirm) {
            ...
            if (pos != 0 && url.query[pos - 1] == '&') {
                pos--;
                len++;
            } // if helper is not only one query option
        ...
        void HTTPReqHandler::SanitizeHTTPRequest(i2p::http::HTTPReq &req) {
            ...
            if (req.GetHeader("Referer") != "") {
                ...
                i2p::http::URL refURL;
                refURL.parse(req.GetHeader("Referer"));
                if (!boost::iequals(reqURL.schema, refURL.schema) || !boost::iequals(reqURL.host, refURL.host) ||
                    reqURL.port != refURL.port)
        ...
        bool HTTPReqHandler::HandleRequest() {
            ...
            if (ExtractAddressHelper(m_RequestURL, jump, m_Confirm)) {
                ...
                if (!i2p::client::context.GetAddressBook().FindAddress(m_RequestURL.host) || m_Confirm) {
                    ...
                    std::stringstream ss;
                    ss << tr("Host") << " " << m_RequestURL.host << " "
                       << tr("added to router's addressbook from helper") << ". ";
                    ss << tr("Click here to proceed:") << " <a href=\"" << full_url << "\">" << tr("Continue")
                       << "</a>.";
                    ...
                    std::string full_url = m_RequestURL.to_string();
                    std::stringstream ss;
                    ss << tr("Host") << " " << m_RequestURL.host << " <font color=red>"
                       << tr("already in router's addressbook") << "</font>. ";
                    ss
                            << tr(/* tr: The "record" means addressbook's record. That message appears when domain was already added to addressbook, but helper link is opened for it. */
                                    "Click here to update record:") << " <a href=\"" << full_url
                            << (full_url.find('?') != std::string::npos ? "&i2paddresshelper=" : "?i2paddresshelper=");
            ...
            if (str_rmatch(dest_host, ".i2p")) {
                ...
                if (m_OutproxyUrl.size()) {
                    ...
                    std::stringstream ss;
                    ss << tr("Host") << " " << dest_host << " "
                       << tr("not inside I2P network, but outproxy is not enabled");
        ...
        void HTTPReqHandler::ForwardToUpstreamProxy() {
            ...
            if (m_ClientRequest.method != "CONNECT")
                m_ClientRequest.UpdateHeader("User-Agent",
            ...
            if (m_ProxyURL.schema == "" || m_ProxyURL.schema == "http") {
                ...
                if (m_ProxyURL.is_i2p()) {
                    ...
                    m_send_buf.append(m_recv_buf);
                    GetOwner()->CreateStream(std::bind(&HTTPReqHandler::HandleStreamRequestComplete,
                                                       shared_from_this(), std::placeholders::_1), m_ProxyURL.host,
                                             m_ProxyURL.port);
                } else {
                    boost::asio::ip::tcp::resolver::query q(m_ProxyURL.host, std::to_string(m_ProxyURL.port));
                    m_proxy_resolver.async_resolve(q, std::bind(&HTTPReqHandler::HandleUpstreamProxyResolved, this,
                                                                std::placeholders::_1, std::placeholders::_2,
                                                                [&](boost::asio::ip::tcp::endpoint ep) {
                                                                    m_proxysock->async_connect(ep, std::bind(
                                                                            &HTTPReqHandler::HandleUpstreamHTTPProxyConnect,
                ...
                m_proxy_resolver.async_resolve(q, std::bind(&HTTPReqHandler::HandleUpstreamProxyResolved, this,
                                                            ...
                                                            [&](boost::asio::ip::tcp::endpoint ep) {
                                                                m_proxysock->async_connect(ep, std::bind(
                                                                        &HTTPReqHandler::HandleUpstreamSocksProxyConnect,
                                                                        this, std::placeholders::_1));
                                                            }));
            } else {
        ...
        void HTTPReqHandler::HandleUpstreamProxyResolved(const boost::system::error_code &ec,
        ...
        void HTTPReqHandler::HandleUpstreamSocksProxyConnect(const boost::system::error_code &ec) {
            if (!ec) {
                ...
                boost::asio::async_write(*m_proxysock, boost::asio::buffer(m_socks_buf, reqsize),
                                         boost::asio::transfer_all(),
                                         std::bind(&HTTPReqHandler::HandleSocksProxySendHandshake, this,
                                                   std::placeholders::_1, std::placeholders::_2));
            } else GenericProxyError(tr("cannot connect to upstream socks proxy"), ec.message());
        }

        void HTTPReqHandler::HandleSocksProxySendHandshake(const boost::system::error_code &ec,
            ...
            if (ec) GenericProxyError(tr("Cannot negotiate with socks proxy"), ec.message());
                ...
                m_proxysock->async_read_some(boost::asio::buffer(m_socks_buf, 8),
                                             std::bind(&HTTPReqHandler::HandleSocksProxyReply, this,
                                                       std::placeholders::_1, std::placeholders::_2));
        }
        ...
        void HTTPReqHandler::SocksProxySuccess() {
            if (m_ClientRequest.method == "CONNECT") {
                m_ClientResponse.code = 200;
                m_send_buf = m_ClientResponse.to_string();
                boost::asio::async_write(*m_sock, boost::asio::buffer(m_send_buf), boost::asio::transfer_all(),
                ...
                boost::asio::async_write(*m_proxysock, boost::asio::buffer(m_send_buf), boost::asio::transfer_all(),
                                         [&](const boost::system::error_code &ec, std::size_t transferred) {
                                             if (ec)
                                                 GenericProxyError(tr("failed to send request to upstream"),
                                                                   ec.message());
                                             else HandoverToUpstreamProxy();
                                         });
            }
        ...
        void HTTPReqHandler::HandleSockRecv(const boost::system::error_code &ecode, std::size_t len) {
            LogPrint(eLogDebug, "HTTPProxy: Sock recv: ", len, " bytes, recv buf: ", m_recv_buf.length(),
                     ", send buf: ", m_send_buf.length());
            if (ecode) {
        ...
        void HTTPReqHandler::HandleStreamRequestComplete(std::shared_ptr <i2p::stream::Stream> stream) {
            if (!stream) {
                LogPrint(eLogError,
                         "HTTPProxy: Error when creating the stream, check the previous warnings for more info");
                GenericProxyError(tr("Host is down"),
                                  tr("Can't create connection to requested host, it may be down. Please try again later."));
                return;
            }
            if (Kill())
                return;
            LogPrint(eLogDebug, "HTTPProxy: Created new I2PTunnel stream, sSID=", stream->GetSendStreamID(), ", rSID=",
        ...
        HTTPProxy::HTTPProxy(const std::string &name, const std::string &address, int port, const std::string &outproxy,
                             bool addresshelper, std::shared_ptr <i2p::client::ClientDestination> localDestination) :
                TCPIPAcceptor(address, port,
        ...
        HTTPProxy::CreateHandler(std::shared_ptr <boost::asio::ip::tcp::socket> socket) {
