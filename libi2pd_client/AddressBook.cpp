...
namespace i2p {
    namespace client {
        ...
        public:
            ...
            void
            SaveEtag(const i2p::data::IdentHash &subsciption, const std::string &etag, const std::string &lastModified);
        ...
        private:
            ...
            int LoadFromFile(const std::string &filename,
                             std::map <std::string, std::shared_ptr<Address>> &addresses); // returns -1 if can't open file, otherwise number of records
        ...
        std::shared_ptr<const i2p::data::IdentityEx>
        AddressBookFilesystemStorage::GetAddress(const i2p::data::IdentHash &ident) const {
            if (!m_IsPersist) {
        ...
        int AddressBookFilesystemStorage::LoadFromFile(const std::string &filename,
                                                       std::map <std::string, std::shared_ptr<Address>> &addresses) {
            int num = 0;
        ...
        void AddressBookFilesystemStorage::SaveEtag(const i2p::data::IdentHash &subscription, const std::string &etag,
        ...
        bool AddressBookFilesystemStorage::GetEtag(const i2p::data::IdentHash &subscription, std::string &etag,
        ...
        void AddressBookFilesystemStorage::ResetEtags() {
            ...
            for (boost::filesystem::directory_iterator it(etagsPath);
...
//---------------------------------------------------------------------
        ...
        AddressBook::AddressBook() : m_Storage(nullptr), m_IsLoaded(false), m_IsDownloading(false),
                                     m_NumRetries(0), m_DefaultSubscription(nullptr),
        ...
        bool AddressBook::LoadHostsFromStream(std::istream &f, bool is_update) {
            ...
            while (!f.eof()) {
                ...
                if (pos != std::string::npos) {
                    ...
                    {
                        if (it->second->IsIdentHash() && it->second->identHash != ident->GetIdentHash() &&
                            // address changed?
            ...
            if (!m_Subscriptions.size()) {
                ...
                if (f.is_open()) {
                    ...
                    LogPrint(eLogWarning,
                             "Addressbook: subscriptions.txt usage is deprecated, use config file instead");
                } else {
                    LogPrint(eLogInfo, "Addressbook: Loading subscriptions from config file");
                    // using config file items
                    std::string subscriptionURLs;
                    i2p::config::GetOption("addressbook.subscriptions", subscriptionURLs);
        ...
        bool
        AddressBook::GetEtag(const i2p::data::IdentHash &subscription, std::string &etag, std::string &lastModified) {
            if (m_Storage)
        ...
        void
        AddressBook::DownloadComplete(bool success, const i2p::data::IdentHash &subscription, const std::string &etag,
                                      const std::string &lastModified) {
            m_IsDownloading = false;
            m_NumRetries++;
            int nextUpdateTimeout = m_NumRetries * CONTINIOUS_SUBSCRIPTION_RETRY_TIMEOUT;
            if (m_NumRetries > CONTINIOUS_SUBSCRIPTION_MAX_NUM_RETRIES ||
            ...
            if (ecode != boost::asio::error::operation_aborted) {
                ...
                if (!m_IsDownloading && dest->IsReady()) {
                    if (!m_IsLoaded) {
                        ...
                        std::string defaultSubURL;
                        i2p::config::GetOption("addressbook.defaulturl", defaultSubURL);
                        if (!m_DefaultSubscription)
                            m_DefaultSubscription = std::make_shared<AddressBookSubscription>(*this, defaultSubURL);
                        m_IsDownloading = true;
                        std::thread
                        load_hosts(std::bind(&AddressBookSubscription::CheckUpdates, m_DefaultSubscription));
                        ...
                        m_IsDownloading = true;
                        std::thread
                        load_hosts(std::bind(&AddressBookSubscription::CheckUpdates, m_Subscriptions[ind]));
                        load_hosts.detach(); // TODO: use join
                    }
                } else {
                    // try it again later
                    m_SubscriptionsUpdateTimer->expires_from_now(
                            boost::posix_time::minutes(INITIAL_SUBSCRIPTION_RETRY_TIMEOUT));
                    m_SubscriptionsUpdateTimer->async_wait(std::bind(&AddressBook::HandleSubscriptionsUpdateTimer,
                                                                     this, std::placeholders::_1));
                }
            }
        ...
        void AddressBook::LookupAddress(const std::string &address) {
            ...
            if (dest) {
                ...
                if (datagram) {
                    ...
                    LogPrint(eLogDebug, "Addressbook: Lookup of ", address, " to ", addr->identHash.ToBase32(),
                             " nonce=", nonce);
                    ...
                    datagram->SendDatagramTo(buf, len, addr->identHash, ADDRESS_RESPONSE_DATAGRAM_PORT,
        ...
        void AddressBook::HandleLookupResponse(const i2p::data::IdentityEx &from, uint16_t fromPort, uint16_t toPort,
            ...
            LogPrint(eLogDebug, "Addressbook: Lookup response received from ", from.GetIdentHash().ToBase32(),
                     " nonce=", nonce);
            std::string address;
            {
                std::unique_lock <std::mutex> l(m_LookupsMutex);
        ...
        bool AddressBookSubscription::MakeRequest() {
            ...
            if (!leaseSet) {
                ...
                i2p::client::context.GetSharedLocalDestination()->RequestDestination(m_Ident,
                                                                                     [&newDataReceived, &leaseSet, &newDataReceivedMutex](
                                                                                             std::shared_ptr <i2p::data::LeaseSet> ls) {
                                                                                         leaseSet = ls;
                                                                                         std::unique_lock <std::mutex> l1(
                                                                                                 newDataReceivedMutex);
                ...
                if (newDataReceived.wait_for(l, std::chrono::seconds(SUBSCRIPTION_REQUEST_TIMEOUT)) ==
                    ...
                    i2p::client::context.GetSharedLocalDestination()->CancelDestinationRequest(m_Ident,
                                                                                               false); // don't notify, because we know it already
                    return false;
                }
            }
            if (!leaseSet) {
            ...
            if (m_Etag.empty() && m_LastModified.empty()) {
                ...
                LogPrint(eLogDebug, "Addressbook: Loaded for ", url.host, ": ETag: ", m_Etag, ", Last-Modified: ",
                         m_LastModified);
            }
            ...
            while (!end) {
                ...
                std::unique_lock <std::mutex> l(newDataReceivedMutex);
                // wait 1 more second
                if (newDataReceived.wait_for(l, std::chrono::seconds(SUBSCRIPTION_REQUEST_TIMEOUT + 1)) ==
                    std::cv_status::timeout) {
            ...
            if (res_head_len == 0) {
                LogPrint(eLogError, "Addressbook: Incomplete http response from ", dest_host,
            ...
            if (!res.is_gzipped() && len > 0 && len != (int) response.length()) {
                LogPrint(eLogError, "Addressbook: Response size mismatch, expected: ", len, ", got: ",
        ...
        void AddressResolver::HandleRequest(const i2p::data::IdentityEx &from, uint16_t fromPort, uint16_t toPort,
                                            const uint8_t *buf, size_t len) {
            ...
            m_LocalDestination->GetDatagramDestination()->SendDatagramTo(response, 44, from.GetIdentHash(), toPort,
                                                                         fromPort);
        }
