...
namespace i2p {
    namespace transport {
        ...
        void SSU2Session::HandleConnectTimer(const boost::system::error_code &ecode) {
            if (!ecode) {
                // timeout expired
                LogPrint(eLogWarning, "SSU2: Session with ", m_RemoteEndpoint, " was not established after ",
                         SSU2_CONNECT_TIMEOUT, " seconds");
                Terminate();
            }
        }
        ...
        bool SSU2Session::Introduce(std::shared_ptr<SSU2Session> session, uint32_t relayTag) {
            ...
            payload[16] = 2; // ver
            size_t asz = CreateEndpoint(payload + 18, m_MaxPayloadSize - 18,
                                        boost::asio::ip::udp::endpoint(localAddress->host, localAddress->port));
            if (!asz) return false;
            payload[17] = asz;
            payloadSize += asz + 18;
            SignedData s;
        ...
        void SSU2Session::SendLocalRouterInfo(bool update) {
            if (update || !IsOutgoing()) {
                ...
                m_Server.GetService().post([s]() {
                    ...
                    uint8_t payload[SSU2_MAX_PACKET_SIZE];
                    size_t payloadSize = s->CreateRouterInfoBlock(payload, s->m_MaxPayloadSize - 32,
                                                                  i2p::context.GetSharedRouterInfo());
                    if (payloadSize) {
                        if (payloadSize < s->m_MaxPayloadSize)
                            payloadSize += s->CreatePaddingBlock(payload + payloadSize,
                                                                 s->m_MaxPayloadSize - payloadSize);
            ...
            if (!m_SendQueue.empty() && m_SentPackets.size() <= m_WindowSize) {
                ...
                while (!m_SendQueue.empty() && m_SentPackets.size() <= m_WindowSize) {
                    ...
                    if (len > m_MaxPayloadSize) // message too long
                    {
                        ...
                        m_SendQueue.pop_front();
                        packet->payloadSize += CreateI2NPBlock(packet->payload + packet->payloadSize,
                                                               m_MaxPayloadSize - packet->payloadSize, std::move(msg));
                    } else {
                        ...
                        if (packet->payloadSize > ackBlockSize) // more than just ack block
                        {
                            ...
                            if (packet->payloadSize + 16 < m_MaxPayloadSize)
                                packet->payloadSize += CreatePaddingBlock(packet->payload + packet->payloadSize,
                                                                          m_MaxPayloadSize - packet->payloadSize);
                        } else {
                            // reduce ack block
        ...
        bool SSU2Session::SendFragmentedMessage(std::shared_ptr<I2NPMessage> msg) {
            ...
            auto size = CreateFirstFragmentBlock(packet->payload + packet->payloadSize,
                                                 m_MaxPayloadSize - offset - packet->payloadSize, msg);
            ...
            while (msg->offset < msg->len) {
                ...
                packet = m_Server.GetSentPacketsPool().AcquireShared();
                packet->payloadSize = CreateFollowOnFragmentBlock(packet->payload, m_MaxPayloadSize - offset, msg,
                                                                  fragmentNum, msgID);
                extraSize -= offset;
            ...
            for (auto it = m_SentPackets.begin(); it != m_SentPackets.end();)
                if (ts >= it->second->sendTime + it->second->numResends * m_RTO) {
                    if (it->second->numResends > SSU2_MAX_NUM_RESENDS) {
                        LogPrint(eLogInfo, "SSU2: Packet was not Acked after ", it->second->numResends,
                                 " attempts. Terminate session");
                        m_SentPackets.clear();
                        m_SendQueue.clear();
                        RequestTermination(eSSU2TerminationReasonTimeout);
                        return;
            ...
            if (m_SentHandshakePacket) {
                ...
                if (m_SessionConfirmedFragment && m_State == eSSU2SessionStateSessionConfirmedSent)
                    ...
                    m_Server.Send(m_SessionConfirmedFragment->header.buf, 16,
                                  m_SessionConfirmedFragment->payload, m_SessionConfirmedFragment->payloadSize,
                                  m_RemoteEndpoint);
            }
        }
        ...
        bool SSU2Session::ProcessFirstIncomingMessage(uint64_t connID, uint8_t *buf, size_t len) {
            ...
            switch (header.h.type) {
                ...
                case eSSU2PeerTest: {
                    ...
                    LogPrint(eLogWarning, "SSU2: Unexpected PeerTest message SourceConnID=", connID, " DestConnID=",
                             headerX[0]);
                    break;
                }
                case eSSU2HolePunch:
                    ...
                    break;
                default: {
                    LogPrint(eLogWarning, "SSU2: Unexpected message type ", (int) header.h.type, " from ",
                             m_RemoteEndpoint, " of ", len, " bytes");
                    return false;
                }
            }
            return true;
        ...
        void SSU2Session::SendSessionRequest(uint64_t token) {
            ...
            m_NoiseState->MixHash({{header.buf, 16},
                                   {headerX,    16}}); // h = SHA256(h || header)
            ...
            // encrypt
            const uint8_t nonce[12] = {0};
            i2p::crypto::AEADChaCha20Poly1305(payload, payloadSize, m_NoiseState->m_H, 32, m_NoiseState->m_CK + 32,
                                              nonce, payload, payloadSize + 16, true);
            payloadSize += 16;
            ...
            m_NoiseState->MixHash(payload,
                                  payloadSize); // h = SHA256(h || encrypted payload from Session Request) for SessionCreated
        ...
        void SSU2Session::ProcessSessionRequest(Header &header, uint8_t *buf, size_t len) {
            ...
            if (!i2p::crypto::AEADChaCha20Poly1305(payload, len - 80, m_NoiseState->m_H, 32,
                                                   m_NoiseState->m_CK + 32, nonce, decryptedPayload.data(),
                                                   decryptedPayload.size(), false)) {
                LogPrint(eLogWarning, "SSU2: SessionRequest AEAD verification failed ");
                return;
            }
            m_NoiseState->MixHash(payload, len -
                                           64); // h = SHA256(h || encrypted payload from Session Request) for SessionCreated
            // payload
        ...
        void SSU2Session::SendSessionCreated(const uint8_t *X) {
            ...
            uint8_t kh2[32];
            i2p::crypto::HKDF(m_NoiseState->m_CK, nullptr, 0, "SessCreateHeader", kh2,
                              32); // k_header_2 = HKDF(chainKey, ZEROLEN, "SessCreateHeader", 32)
            // fill packet
            Header &header = m_SentHandshakePacket->header;
            uint8_t *headerX = m_SentHandshakePacket->headerX,
                    *payload = m_SentHandshakePacket->payload;
            ...
            payloadSize += 16;
            m_NoiseState->MixHash(payload,
                                  payloadSize); // h = SHA256(h || encrypted Noise payload from Session Created)
        ...
        bool SSU2Session::ProcessSessionCreated(uint8_t *buf, size_t len) {
            ...
            m_NoiseState->MixHash(payload, len -
                                           64); // h = SHA256(h || encrypted payload from SessionCreated) for SessionConfirmed
            // payload
        ...
        void SSU2Session::SendSessionConfirmed(const uint8_t *Y) {
            ...
            uint8_t kh2[32];
            i2p::crypto::HKDF(m_NoiseState->m_CK, nullptr, 0, "SessionConfirmed", kh2,
                              32); // k_header_2 = HKDF(chainKey, ZEROLEN, "SessionConfirmed", 32)
            // fill packet
            Header &header = m_SentHandshakePacket->header;
            header.h.connID = m_DestConnID; // dest id
            header.h.packetNum = 0;
            ...
            uint8_t nonce[12];
            CreateNonce(1, nonce);
            i2p::crypto::AEADChaCha20Poly1305(i2p::context.GetSSU2StaticPublicKey(), 32, m_NoiseState->m_H, 32,
                                              m_NoiseState->m_CK + 32, nonce, part1, 48, true);
            ...
            if (m_SentHandshakePacket->payloadSize > payloadSize) {
                ...
                memcpy(m_SessionConfirmedFragment->payload, m_SentHandshakePacket->payload + payloadSize,
                       m_SessionConfirmedFragment->payloadSize);
                m_SentHandshakePacket->payloadSize = payloadSize;
                header.ll[0] ^= CreateHeaderMask(m_Address->i, m_SessionConfirmedFragment->payload +
                                                               (m_SessionConfirmedFragment->payloadSize - 24));
                header.ll[1] ^= CreateHeaderMask(kh2, m_SessionConfirmedFragment->payload +
                                                      (m_SessionConfirmedFragment->payloadSize - 12));
                m_Server.Send(header.buf, 16, m_SessionConfirmedFragment->payload,
                              m_SessionConfirmedFragment->payloadSize, m_RemoteEndpoint);
            }
        }
        ...
        bool SSU2Session::ProcessSessionConfirmed(uint8_t *buf, size_t len) {
            ...
            if (header.h.type != eSSU2SessionConfirmed) {
                LogPrint(eLogInfo, "SSU2: Unexpected message type ", (int) header.h.type, " instead ",
                         (int) eSSU2SessionConfirmed);
                // TODO: queue up
                return true;
            }
            // check if fragmented
            if ((header.h.flags[0] & 0x0F) > 1) {
                ...
                if (!(header.h.flags[0] & 0xF0)) {
                    ...
                    if (!m_SessionConfirmedFragment) {
                        ...
                        m_SessionConfirmedFragment->header = header;
                        memmove(m_SessionConfirmedFragment->payload + (len - 16), m_SessionConfirmedFragment->payload,
                                m_SessionConfirmedFragment->payloadSize);
                        memcpy(m_SessionConfirmedFragment->payload, buf + 16, len - 16);
                        m_SessionConfirmedFragment->payloadSize += (len - 16);
                    ...
                    memcpy(m_SessionConfirmedFragment->payload + m_SessionConfirmedFragment->payloadSize, buf + 16,
                           len - 16);
                    m_SessionConfirmedFragment->payloadSize += (len - 16);
                    buf = m_SessionConfirmedFragment->payload - 16;
                    len = m_SessionConfirmedFragment->payloadSize + 16;
            ...
            if (!m_Address) {
                LogPrint(eLogError, "SSU2: No SSU2 address with static key found in SessionConfirmed from ",
        ...
        void SSU2Session::KDFDataPhase(uint8_t *keydata_ab, uint8_t *keydata_ba) {
            ...
            // ab
            i2p::crypto::HKDF(keydata, nullptr, 0, "HKDFSSU2DataKeys",
                              keydata_ab); // keydata_ab = HKDF(keydata, ZEROLEN, "HKDFSSU2DataKeys", 64)
            // ba
            i2p::crypto::HKDF(keydata + 32, nullptr, 0, "HKDFSSU2DataKeys",
                              keydata_ba); // keydata_ba = HKDF(keydata + 32, ZEROLEN, "HKDFSSU2DataKeys", 64)
        }
        ...
        void SSU2Session::SendTokenRequest() {
            ...
            i2p::crypto::AEADChaCha20Poly1305(payload, payloadSize, h, 32, m_Address->i, nonce, payload,
                                              payloadSize + 16, true);
            payloadSize += 16;
            header.ll[0] ^= CreateHeaderMask(m_Address->i, payload + (payloadSize - 24));
            header.ll[1] ^= CreateHeaderMask(m_Address->i, payload + (payloadSize - 12));
        ...
        void SSU2Session::SendRetry() {
            ...
            uint8_t nonce[12];
            CreateNonce(be32toh(header.h.packetNum), nonce);
            i2p::crypto::AEADChaCha20Poly1305(payload, payloadSize, h, 32, i2p::context.GetSSU2IntroKey(), nonce,
                                              payload, payloadSize + 16, true);
            payloadSize += 16;
            header.ll[0] ^= CreateHeaderMask(i2p::context.GetSSU2IntroKey(), payload + (payloadSize - 24));
        ...
        bool SSU2Session::ProcessRetry(uint8_t *buf, size_t len) {
            ...
            if (header.h.type != eSSU2Retry) {
                LogPrint(eLogWarning, "SSU2: Unexpected message type ", (int) header.h.type, " instead ",
                         (int) eSSU2Retry);
                return false;
            }
            uint8_t nonce[12] = {0};
            ...
            if (token)
                m_Server.UpdateOutgoingToken(m_RemoteEndpoint, token,
                                             i2p::util::GetSecondsSinceEpoch() + SSU2_TOKEN_EXPIRATION_TIMEOUT);
        ...
        void SSU2Session::SendHolePunch(uint32_t nonce, const boost::asio::ip::udp::endpoint &ep,
            ...
            i2p::crypto::AEADChaCha20Poly1305(payload, payloadSize, h, 32, introKey, n, payload, payloadSize + 16,
                                              true);
            payloadSize += 16;
        ...
        void SSU2Session::SendPeerTest(uint8_t msg, const uint8_t *signedData, size_t signedDataLen,
            ...
            size_t payloadSize = 7;
            if (msg == 6 || msg == 7)
                payloadSize += CreateAddressBlock(payload + payloadSize, m_MaxPayloadSize - payloadSize,
                                                  m_RemoteEndpoint);
        ...
        uint32_t SSU2Session::SendData(const uint8_t *buf, size_t len) {
            ...
            uint8_t payload[SSU2_MAX_PACKET_SIZE];
            i2p::crypto::AEADChaCha20Poly1305(buf, len, header.buf, 16, m_KeyDataSend, nonce, payload,
                                              SSU2_MAX_PACKET_SIZE, true);
            ...
            while (offset < len) {
                ...
                switch (blk) {
                    ...
                    case eSSU2BlkI2NPMessage: {
                        ...
                        auto nextMsg = (buf[offset] == eI2NPTunnelData) ? NewI2NPTunnelMessage(true)
                                                                        : NewI2NPShortMessage();
        ...
        void SSU2Session::HandleAck(const uint8_t *buf, size_t len) {
            ...
            while (len > 0 && firstPacketNum) {
                ...
                lastPacketNum -= *ranges;
                ranges++; // nacks
                if (*ranges > lastPacketNum + 1) break;
                firstPacketNum = lastPacketNum - *ranges + 1;
                ranges++; // acks
                len -= 2;
            ...
            if (ExtractEndpoint(buf, len, ep)) {
                ...
                if (!i2p::util::net::IsInReservedRange(ep.address())) {
                    ...
                    if (ep.port() != m_Server.GetPort(isV4)) {
                        ...
                        if (isV4) {
                            if (i2p::context.GetStatus() == eRouterStatusError &&
                                i2p::context.GetError() == eRouterErrorSymmetricNAT)
                                i2p::context.SetStatus(eRouterStatusTesting);
                        } else {
                            if (i2p::context.GetStatusV6() == eRouterStatusError &&
                                i2p::context.GetErrorV6() == eRouterErrorSymmetricNAT)
        ...
        void SSU2Session::HandleRelayRequest(const uint8_t *buf, size_t len) {
            ...
            payloadSize += CreateRelayIntroBlock(payload + payloadSize, m_MaxPayloadSize - payloadSize, buf + 1,
                                                 len - 1);
                            ...
                            if (it->second.first->m_State == eSSU2SessionStateIntroduced) // HolePunch not received yet
                            {
                                ...
                                if (ExtractEndpoint(buf + 12, csz, it->second.first->m_RemoteEndpoint)) {
                                    ...
                                    m_Server.UpdateOutgoingToken(it->second.first->m_RemoteEndpoint,
                                                                 token, i2p::util::GetSecondsSinceEpoch() +
        ...
        void SSU2Session::HandlePeerTest(const uint8_t *buf, size_t len) {
            ...
            switch (msg) // msg
            {
                ...
                {
                    auto session = m_Server.GetRandomSession(
                            (buf[12] == 6) ? i2p::data::RouterInfo::eSSU2V4 : i2p::data::RouterInfo::eSSU2V6,
                            GetRemoteIdentity()->GetIdentHash());
                    if (session) // session with Charlie
                    {
                        session->m_PeerTests.emplace(nonce, std::make_pair(shared_from_this(),
                        ...
                        payloadSize += CreatePeerTestBlock(payload + payloadSize, m_MaxPayloadSize - payloadSize, 2,
                                                           eSSU2PeerTestCodeAccept, GetRemoteIdentity()->GetIdentHash(),
                        ...
                        size_t payloadSize = CreatePeerTestBlock(payload, m_MaxPayloadSize, 4,
                                                                 eSSU2PeerTestCodeBobNoCharlieAvailable, zeroHash,
                                                                 buf + offset, len - offset);
                        payloadSize += CreatePaddingBlock(payload + payloadSize, m_MaxPayloadSize - payloadSize);
                        SendData(payload, payloadSize);
                    }
                ...
                case 3: // Bob from Charlie
                {
                    ...
                    if (it != m_PeerTests.end() && it->second.first) {
                        ...
                        payloadSize += CreatePeerTestBlock(payload + payloadSize, m_MaxPayloadSize, 4,
                                                           (SSU2PeerTestCode) buf[1],
                                                           GetRemoteIdentity()->GetIdentHash(), buf + offset,
                                                           len - offset);
                        if (payloadSize < m_MaxPayloadSize)
                            payloadSize += CreatePaddingBlock(payload + payloadSize, m_MaxPayloadSize - payloadSize);
                        it->second.first->SendData(payload, payloadSize);
                        ...
                        if (buf[1] == eSSU2PeerTestCodeAccept) {
                            ...
                            LogPrint(eLogInfo, "SSU2: Peer test 4 error code ", (int) buf[1], " from ",
                                     i2p::data::GetIdentHashAbbreviation(
                                             buf[1] < 64 ? GetRemoteIdentity()->GetIdentHash() : i2p::data::IdentHash(
                                                     buf + 3)));
                            if (GetRouterStatus() == eRouterStatusTesting)
        ...
        void SSU2Session::AdjustMaxPayloadSize() {
            ...
            if (addr && addr->ssu) {
                ...
                if (mtu) {
                    m_MaxPayloadSize =
                            mtu - (addr->IsV6() ? IPV6_HEADER_SIZE : IPV4_HEADER_SIZE) - UDP_HEADER_SIZE - 32;
        ...
        size_t
        SSU2Session::CreateRouterInfoBlock(uint8_t *buf, size_t len, std::shared_ptr<const i2p::data::RouterInfo> r) {
        ...
        size_t SSU2Session::CreateAckBlock(uint8_t *buf, size_t len) {
            ...
            buf[0] = eSSU2BlkAck;
            uint32_t ackThrough = m_OutOfSequencePackets.empty() ? m_ReceivePacketNum
                                                                 : *m_OutOfSequencePackets.rbegin();
            ...
            if (ackThrough) {
                if (m_OutOfSequencePackets.empty())
                    ...
                    auto it = m_OutOfSequencePackets.rbegin();
                    it++; // prev packet num
                    ...
                    if (acnt > 255) {
                        ...
                        for (int i = 0; i < d.quot; i++) {
                            buf[8 + numRanges * 2] = 0;
                            buf[8 + numRanges * 2 + 1] = 255; // NACKs 0, Acks 255
                            numRanges++;
                        }
                        if (d.rem > 0) {
                            buf[8 + numRanges * 2] = 0;
                            buf[8 + numRanges * 2 + 1] = d.rem;
                            numRanges++;
                        }
                    }
                    while (it != m_OutOfSequencePackets.rend() && numRanges < maxNumRanges) {
                        if (lastNum - (*it) > 255) {
                            ...
                            while (lastNum - (*it) > 255) {
                                buf[8 + numRanges * 2] = 255;
                                buf[8 + numRanges * 2 + 1] = 0; // NACKs 255, Acks 0
                                lastNum -= 255;
                                numRanges++;
                            }
                        }
        ...
        size_t SSU2Session::CreateFollowOnFragmentBlock(uint8_t *buf, size_t len, std::shared_ptr<I2NPMessage> msg,
        ...
        size_t
        SSU2Session::CreateRelayIntroBlock(uint8_t *buf, size_t len, const uint8_t *introData, size_t introDataLen) {
        ...
        size_t SSU2Session::CreateRelayResponseBlock(uint8_t *buf, size_t len,
                                                     SSU2RelayResponseCode code, uint32_t nonce, uint64_t token,
                                                     bool v4) {
        ...
        size_t SSU2Session::CreatePeerTestBlock(uint8_t *buf, size_t len, uint8_t msg, SSU2PeerTestCode code,
                                                const uint8_t *routerHash, const uint8_t *signedData,
        ...
        size_t SSU2Session::CreatePeerTestBlock(uint8_t *buf, size_t len, uint32_t nonce) {
            ...
            size_t asz = CreateEndpoint(signedData + 10, 86,
                                        boost::asio::ip::udp::endpoint(localAddress->host, localAddress->port));
            signedData[9] = asz;
            // signature
            SignedData s;
        ...
        std::shared_ptr<const i2p::data::RouterInfo> SSU2Session::ExtractRouterInfo(const uint8_t *buf, size_t size) {
            ...
            if (buf[0] & SSU2_ROUTER_INFO_FLAG_GZIP) {
                ...
                size_t uncompressedSize = inflator.Inflate(buf + 2, size - 2, uncompressed,
                                                           i2p::data::MAX_RI_BUFFER_SIZE);
        ...
        void SSU2Session::CleanUp(uint64_t ts) {
            for (auto it = m_IncompleteMessages.begin(); it != m_IncompleteMessages.end();) {
                if (ts > it->second->lastFragmentInsertTime + SSU2_INCOMPLETE_MESSAGES_CLEANUP_TIMEOUT) {
                    LogPrint(eLogWarning, "SSU2: message ", it->first, " was not completed in ",
                             SSU2_INCOMPLETE_MESSAGES_CLEANUP_TIMEOUT, " seconds, deleted");
                ...
                if (m_OutOfSequencePackets.size() > 2 * SSU2_MAX_NUM_ACK_RANGES ||
                    *m_OutOfSequencePackets.rbegin() > m_ReceivePacketNum + 255 * 8) {
                    ...
                    if (packet > m_ReceivePacketNum + 1) {
                        ...
                        LogPrint(eLogError, "SSU2: Out of sequence packet ", packet, " is less than last received ",
                                 m_ReceivePacketNum);
                }
            ...
            for (auto it = m_RelaySessions.begin(); it != m_RelaySessions.end();) {
                if (ts > it->second.second + SSU2_RELAY_NONCE_EXPIRATION_TIMEOUT) {
                    LogPrint(eLogWarning, "SSU2: Relay nonce ", it->first, " was not responded in ",
                             SSU2_RELAY_NONCE_EXPIRATION_TIMEOUT, " seconds, deleted");
            ...
            for (auto it = m_PeerTests.begin(); it != m_PeerTests.end();) {
                if (ts > it->second.second + SSU2_PEER_TEST_EXPIRATION_TIMEOUT) {
                    LogPrint(eLogWarning, "SSU2: Peer test nonce ", it->first, " was not responded in ",
                             SSU2_PEER_TEST_EXPIRATION_TIMEOUT, " seconds, deleted");
