...
namespace i2p {
    namespace transport {
        ...
        void
        SSUSession::ProcessNextMessage(uint8_t *buf, size_t len, const boost::asio::ip::udp::endpoint &senderEndpoint) {
        ...
        void
        SSUSession::ProcessMessage(uint8_t *buf, size_t len, const boost::asio::ip::udp::endpoint &senderEndpoint) {
        ...
        void SSUSession::ProcessSessionRequest(const uint8_t *buf, size_t len) {
            ...
            if (headerSize >= len) {
                LogPrint(eLogError, "SSU message: Session request header size ", headerSize, " exceeds packet length ",
                         len);
                return;
            }
            if (!m_DHKeysPair) {
        ...
        void SSUSession::ProcessSessionCreated(uint8_t *buf, size_t len) {
            ...
            auto headerSize = GetSSUHeaderSize(buf);
            if (headerSize >= len) {
                LogPrint(eLogError, "SSU message: Session created header size ", headerSize, " exceeds packet length ",
                         len);
                return;
            }
        ...
        void SSUSession::ProcessSessionConfirmed(const uint8_t *buf, size_t len) {
            ...
            {
                LogPrint(eLogError, "SSU: Session confirmed identity size ", identitySize, " exceeds packet length ",
                         len);
                return;
            }
            ...
            if (signedOnTime < ts - SSU_CLOCK_SKEW || signedOnTime > ts + SSU_CLOCK_SKEW) {
                LogPrint(eLogError, "SSU: Message 'confirmed' time difference ", (int) ts - signedOnTime,
                         " exceeds clock skew");
                Failed();
                return;
            }
        ...
        void
        SSUSession::ProcessRelayRequest(const uint8_t *buf, size_t len, const boost::asio::ip::udp::endpoint &from) {
        ...
        void SSUSession::SendRelayResponse(uint32_t nonce, const boost::asio::ip::udp::endpoint &from,
            ...
            if ((isV4 && !isV4A) || (!isV4 && isV4A)) {
                LogPrint(eLogWarning,
                         "SSU: Charlie's IP and Alice's IP belong to different networks for relay response");
                return;
            }
        ...
        void
        SSUSession::SendRelayIntro(std::shared_ptr<SSUSession> session, const boost::asio::ip::udp::endpoint &from) {
            if (!session) return;
            ...
            *payload = 0; // challenge size
            uint8_t iv[16];
            RAND_bytes(iv, 16); // random iv
            FillHeaderAndEncrypt(PAYLOAD_TYPE_RELAY_INTRO, buf, isV4 ? 48 : 64, session->m_SessionKey, iv,
                                 session->m_MacKey);
        ...
        void SSUSession::ProcessRelayResponse(const uint8_t *buf, size_t len) {
            ...
            if (ourIP.is_v4()) {
                ...
                } else if (i2p::context.GetStatus() == eRouterStatusError &&
        ...
        void SSUSession::FillHeaderAndEncrypt(uint8_t payloadType, uint8_t *buf, size_t len,
                                              const i2p::crypto::AESKey &aesKey, const uint8_t *iv,
                                              const i2p::crypto::MACKey &macKey, uint8_t flag) {
        ...
        void SSUSession::HandleConnectTimer(const boost::system::error_code &ecode) {
            if (!ecode) {
                // timeout expired
                LogPrint(eLogWarning, "SSU: Session with ", m_RemoteEndpoint, " was not established after ",
                ...
                for (const auto &it: msgs)
                    if (it) {
                        if (it->GetLength() <= SSU_MAX_I2NP_MESSAGE_SIZE)
                            ...
                            LogPrint(eLogError, "SSU: I2NP message of size ", it->GetLength(),
                                     " can't be sent. Dropped");
                    }
            }
        }
        ...
        void SSUSession::ProcessPeerTest(const uint8_t *buf, size_t len,
            ...
            switch (m_Server.GetPeerTestParticipant(nonce)) {
                ...
                case ePeerTestParticipantAlice1: {
                    if (m_Server.GetPeerTestSession(nonce) == shared_from_this()) // Alice-Bob
                    {
                        ...
                        if (m_State == eSessionStateEstablished)
                            LogPrint(eLogWarning,
                                     "SSU: First peer test from Charlie through established session. We are Alice");
                        if (IsV6())
                            i2p::context.SetStatusV6(eRouterStatusOK);
                        else
                            i2p::context.SetStatus(eRouterStatusOK);
                        m_Server.UpdatePeerTest(nonce, ePeerTestParticipantAlice2);
                        SendPeerTest(nonce, senderEndpoint.address(), senderEndpoint.port(), introKey, true,
                                     false); // to Charlie
                    }
                    break;
                }
                ...
                case ePeerTestParticipantBob: {
                    ...
                    if (session && session->m_State == eSessionStateEstablished) {
                        ...
                        session->SendPeerTest(nonce, ep.address(), ep.port(), introKey, false,
                                              true); // send back to Alice
                    }
                ...
                case ePeerTestParticipantCharlie: {
                    ...
                    SendPeerTest(nonce, senderEndpoint.address(), senderEndpoint.port(),
                    ...
                    if (m_State == eSessionStateEstablished) {
                        ...
                        if (port) {
                            ...
                            if (!addr.is_unspecified() && !i2p::util::net::IsInReservedRange(addr)) {
                                ...
                                SendPeerTest(nonce, addr, port,
                                             introKey); // to Alice with her address received from Bob
                            }
                        } else {
                            ...
                            if (session) {
                                ...
                                session->SendPeerTest(nonce, senderEndpoint.address(), senderEndpoint.port(), introKey,
        ...
        void SSUSession::SendPeerTest() {
            ...
            SendPeerTest(nonce, boost::asio::ip::address(), 0, address->i, false,
                         false); // address and port always zero for Alice
        }
        ...
        size_t SSUSession::ExtractIPAddressAndPort(const uint8_t *buf, size_t len, boost::asio::ip::address &ip,
                                                   uint16_t &port) {
            if (!len) return 0;
            uint8_t size = *buf;
