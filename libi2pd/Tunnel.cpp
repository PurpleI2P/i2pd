...
namespace i2p {
    namespace tunnel {
            ...
            *msg->GetPayload() = numRecords;
            ...
            while (hop) {
                ...
                if (hop->next) // we set replyMsgID for last hop only
                    ...
                    msgID = replyMsgID;
                hop->recordIndex = recordIndicies[i];
                i++;
            ...
            if (outboundTunnel) {
                if (m_Config->IsShort()) {
                    ...
                    if (ident &&
                        ident->GetIdentHash() != outboundTunnel->GetNextIdentHash()) // don't encrypt if IBGW = OBEP
                    {
        ...
        void OutboundTunnel::SendTunnelDataMsg(const uint8_t *gwHash, uint32_t gwTunnel,
            ...
            for (auto &msg: msgs) {
                ...
                switch (msg.deliveryType) {
                    ...
                    case eDeliveryTypeTunnel:
                        i2p::transport::transports.SendMessage(msg.hash,
                                                               i2p::CreateTunnelGatewayMsg(msg.tunnelID, msg.data));
                        break;
        ...
        std::shared_ptr<TTunnel> Tunnels::GetPendingTunnel(uint32_t replyMsgID,
                                                           const std::map<uint32_t, std::shared_ptr<TTunnel> > &pendingTunnels) {
        ...
        std::shared_ptr<TunnelPool> Tunnels::CreateTunnelPool(int numInboundHops, int numOutboundHops,
            ...
            auto pool = std::make_shared<TunnelPool>(numInboundHops, numOutboundHops, numInboundTunnels,
                                                     numOutboundTunnels, inboundVariance, outboundVariance);
                            ...
                            switch (typeID) {
                                ...
                                case eI2NPTunnelGateway: {
                                    ...
                                    if (tunnel) {
                                        ...
                                        LogPrint(eLogWarning, "Tunnel: Tunnel not found, tunnelID=", tunnelID,
                                                 " previousTunnelID=", prevTunnelID, " type=", (int) typeID);
        ...
        void Tunnels::HandleTunnelGatewayMsg(std::shared_ptr<TunnelBase> tunnel, std::shared_ptr<I2NPMessage> msg) {
            ...
            LogPrint(eLogDebug, "Tunnel: Gateway of ", (int) len, " bytes for tunnel ", tunnel->GetTunnelID(),
                     ", msg type ", (int) typeID);
        ...
        void Tunnels::ManageOutboundTunnels() {
                ...
                for (auto it = m_OutboundTunnels.begin(); it != m_OutboundTunnels.end();) {
                    ...
                    if (ts > tunnel->GetCreationTime() + TUNNEL_EXPIRATION_TIMEOUT) {
                        ...
                        if (tunnel->IsEstablished()) {
                            if (!tunnel->IsRecreated() && ts + TUNNEL_RECREATION_THRESHOLD >
            ...
            if (m_OutboundTunnels.size() < 3) {
                ...
                auto router = i2p::transport::transports.RoutesRestricted() ?
                              ...
                              i2p::data::netdb.GetRandomRouter(i2p::context.GetSharedRouterInfo(),
                                                               false); // reachable by us
                if (!inboundTunnel || !router) return;
                LogPrint(eLogDebug, "Tunnel: Creating one hop outbound tunnel");
                CreateTunnel<OutboundTunnel>(
                        std::make_shared<TunnelConfig>(
                                std::vector<std::shared_ptr<const i2p::data::IdentityEx> >{router->GetRouterIdentity()},
        ...
        void Tunnels::ManageInboundTunnels() {
            ...
            if (m_InboundTunnels.empty()) {
                ...
                if (!m_ExploratoryPool) {
                    int ibLen;
                    i2p::config::GetOption("exploratory.inbound.length", ibLen);
                    int obLen;
                    i2p::config::GetOption("exploratory.outbound.length", obLen);
                    int ibNum;
                    i2p::config::GetOption("exploratory.inbound.quantity", ibNum);
                    int obNum;
                    i2p::config::GetOption("exploratory.outbound.quantity", obNum);
        ...
        Tunnels::CreateOutboundTunnel(std::shared_ptr<TunnelConfig> config, std::shared_ptr<TunnelPool> pool) {
            if (config)
            ...
            if (m_Tunnels.emplace(newTunnel->GetTunnelID(), newTunnel).second) {
                ...
                if (!pool) {
                    ...
                    CreateTunnel<OutboundTunnel>(std::make_shared<TunnelConfig>(newTunnel->GetInvertedPeers(),
                                                                                newTunnel->GetNextTunnelID(),
                                                                                newTunnel->GetNextIdentHash(), false),
                                                 nullptr,
        ...
        std::shared_ptr<ZeroHopsOutboundTunnel>
        Tunnels::CreateZeroHopsOutboundTunnel(std::shared_ptr<TunnelPool> pool) {
