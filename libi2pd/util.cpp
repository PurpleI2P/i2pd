        ...
        namespace net {
            ...
            int GetMTUUnix(const boost::asio::ip::address &localAddress, int fallback) {
                ...
                if (ifa && family) { // interface found?
                    ...
                    LogPrint(eLogWarning, "NetIface: Interface for local address", localAddress.to_string(),
                             " not found");
                freeifaddrs(ifaddr);
            ...
            boost::asio::ip::address_v6 GetYggdrasilAddress() {
                ...
                try {
                    ...
                    LogPrint(eLogError, "NetIface: Exception while searching Yggdrasill address using ifaddr: ",
                             ex.what());
                }
                LogPrint(eLogWarning, "NetIface: Interface with Yggdrasil network address not found");
                if (addrs) freeifaddrs(addrs);
